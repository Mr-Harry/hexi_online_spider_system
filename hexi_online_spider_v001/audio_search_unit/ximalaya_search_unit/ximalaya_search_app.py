...
class XiMaLaYa(object):
    ...
    def parms_search_songs(self,result):
        ...
        # print(type(info_dic))
        #
        if "data" in info_dic and "trackViews" in info_dic["data"] and "tracks" in info_dic["data"]["trackViews"] and info_dic["data"]["trackViews"]["tracks"]:
            for each in info_dic["data"]["trackViews"]["tracks"]:
                each_info = each.get("pageUriInfo",{})
                each = each["trackInfo"]
                # if int(each["duration"])<350 and each["category_title"]=="音乐":
                if int(each["duration"])<350 :
                    dic_ = {}
                    dic_["audio2_albumName"] = each["album_title"]
                    dic_["audio2_artistName"] = each["nickname"]
                    dic_["audio2_songName"] = each["title"]
                    dic_["audio2_songId"] = each["id"]
                    # dic_["audio2_songtime"] = int(each["created_at"]/1000) # 时间
                    dic_["audio2_songtime"] = datetime.datetime.fromtimestamp(int(each["created_at"]/1000)).strftime("%Y-%m-%d %H:%M:%S") # 时间
                    dic_["audio2_platform"] = "喜马拉雅app"
                    ...
                    dic_["audio2_albumid"] = each["album_id"]
                    ...
                    dic_["audio2_url"] = "https://m.ximalaya.com{share_url}".format(share_url=each_info["url"])
    ...
    def search_songs(self, song_name='在希望的田野上', proxy={}, num=0,**kwargs):
        ...
        # for page in range(config["xi_ma_la_ya"]["start"], config["xi_ma_la_ya"]["end"]):
            ...
            # url = "https://m.ximalaya.com/m-revision/search/main?kw={song_name}&page={page}&spellchecker=true&condition=relation&rows=20&device=iPhone&core=track&fq=category_id%3A2&paidFilter=false".format(song_name=quote(song_name),page=page)
            url = "http://m.ximalaya.com/m-revision/page/search?kw={song_name}&core=track&page={page}&rows=10".format(song_name=quote(song_name),page=page)
            # print(url)
            if proxy:
                self.getSign(self.getServerTime())
...
if __name__ == '__main__':
    ...
    each = {
        ...
        "task_type": 2,
        "page_num": 3,
        "search_key_words": "可可托海的牧羊人",
        # "confirm_key_words": "忘川彼岸",
