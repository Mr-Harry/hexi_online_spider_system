...
class JinRICartoon:
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = int(kwargs.get("page_num",1)) + CARTOON_CONF["jinri_search_offset"]["start_page"] - 1 if int(kwargs.get("page_num",1)) else CARTOON_CONF["jinri_search_offset"]["start_page"]
        # print("xxxx",int(_page))
        # print("xxxx",int(_page)*20)
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(int(_page)*20,quote(search_key)+"+"+quote("漫画")), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        ...
        for q_l in response.json()["data"]:
            if "abstract" in q_l :
                ...
                qin_quan_dict['qin_quan_platform_str'] = '头条站内搜索'  # 侵权平台
                ...
                qin_quan_dict['book_cover_image_str'] = q_l.get("middle_image_url", "") # 书籍封面 URL
...
if __name__ == "__main__":
    ...
    result = search_normals('枪爷异闻录', **yangben_dict)
