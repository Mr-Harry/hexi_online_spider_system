...
class FengHuangVideo:

    def __init__(self, use_proxy=True):
        self.search_video_url = "https://shankapi.ifeng.com/season/getSoFengData/video/{}/{}"
        self.headers = {
    ...
    def search_songs(self, song_name = '', **kwargs) -> list:
        ...
        # 搜索页码search_page
        _start = config["fenghuang_video_search_offset"]["start_page"]
    ...
    def parse_search_video_song(self, response) -> list:
        ...
        try:
            ...
            result_list = []
            try:
                video_song_list_data = tree.get('data', {}).get('items', [])
            except:
                video_song_list_data =[]
            for v_s in video_song_list_data:
                video_dict = dict()
                video_dict["video2_title"] = v_s.get("title", '').replace('<em>', '').replace('</em>', '')
                if not self.check_video_title(video_dict.get("video2_title")):
                    continue
                video_dict["video2_url"] = self.url_add_http(v_s.get("url", ''))
                ...
                video_dict["video2_platform"] = "凤凰视频"
                duration_str_temp = ''
...
search_songs = FengHuangVideo(use_proxy=True).search_songs
