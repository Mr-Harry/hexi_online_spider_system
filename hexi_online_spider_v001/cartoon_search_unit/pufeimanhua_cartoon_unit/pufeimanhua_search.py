...
class PuFeiManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://pufeimanhua.com/sort/?key={}"
        self.qin_quan_search_url_pre = "https://pufeimanhua.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") + CARTOON_CONF["qianweimanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["qianweimanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_url_str = self.qin_quan_search_url_pre  + ''.join(q_l.xpath('./a/@href'))
                author_str = len(''.join(q_l.xpath('./div/p/text()')))-3
                qin_quan_author_str = ''.join(q_l.xpath('./div/p/text()'))[:author_str]    # 侵权作者

                qin_quan_title_str = ''.join(q_l.xpath('./div/h3/a/text()'))  # 侵权标题
                ...
                # qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))[3:]  # 侵权作者
                # qin_quan_author_str = ''    # 侵权作者
                # # if not qin_quan_author_str:
                # #     continue
                # qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="title-warper"]/h1[@class="title"]/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '扑飞漫画'  # 侵权平台
...
search_normals = PuFeiManHua().search_qin_quan
