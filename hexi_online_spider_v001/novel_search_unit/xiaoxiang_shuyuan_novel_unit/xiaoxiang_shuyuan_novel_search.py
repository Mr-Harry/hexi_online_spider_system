...
class QiMaoNovel:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            # 'Cookie': 'ASP.NET_SessionId=qudj105lijfuckshzfpwno2y; pcxxsyseid=f829e3e6b9ea4a3481ff3554a5011fd9'
        }

    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'https://www.xxsy.net/search?&s_wd={}&pn={}'
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["xiaoxiang_search_offset"]["start"]
        _end = NOVEL_CONF["xiaoxiang_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["xiaoxiang_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["xiaoxiang_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//div[@class="result-list"]/ul/li')
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('./div/h4/span/a')[0].xpath('./text()')).replace(' ', '')  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('./div/h4/a/text()')).replace(' ', '')  # 侵权标题
                qin_quan_url_str =  "https://www.xxsy.net" + "".join(n_l.xpath('./div/h4/a/@href')).replace(' ', '') # 侵权链接
                qin_quan_id_int = self.get_qin_quan_id_int(qin_quan_url_str)  # 样本ID 数值类型
                qin_quan_mid_str = str(qin_quan_id_int)  # 侵权ID 字符串形式
                book_type_str = "".join(n_l.xpath('./div/h4/span/a')[1].xpath('./text()')).replace(' ', '')
                ...
                novel_dict['qin_quan_platform_str'] = '潇湘书院'  # 侵权平台
