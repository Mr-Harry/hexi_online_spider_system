...
class HanHanManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.ttlyl.com/search/{}.html"
        self.qin_quan_search_url_pre = "http://www.ttlyl.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") + CARTOON_CONF["Onemanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["Onemanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="newlist"]/li[@class="text-more"]/div[@class="newlist-title text-more xb3 xm3 xs3 xl7"]/a') #
            for q_l in qin_quaq_list_data:
                # xpath_demo = q_l.xpath('//div[@class="newlist-title text-more xb3 xm3 xs3 xl7"]/a/@href')
                ...
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('//p[@class="info-xg clearfix "]/span/a[@class="mtinfo_href"]/text()')[1])   # 侵权作者
                # print(qin_quan_author_str)
                # if not qin_quan_author_str:
                #     continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="info xb10 xm12 xs12 xl8 float-right"]/h2/text()'))  # 侵权标题
...
search_normals = HanHanManHua().search_qin_quan
