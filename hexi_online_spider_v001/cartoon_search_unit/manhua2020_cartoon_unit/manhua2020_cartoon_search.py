...
class ManHua2020Cartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.2020mh.com/search/q_{}-p-{}"
        self.qin_quan_info_url_pre = "http://www.2020mh.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhua2020_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhua2020_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="book-result"]/li[@class="cf"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./div[@class="book-detail"]/dl/dt/a/@href'))  # 侵权链接
                ...
                qin_quan_title_str = ''.join(q_l.xpath('./div[@class="book-detail"]/dl/dt/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '2020漫画'  # 侵权平台
                ...
                qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = ''.join(q_l.xpath('./div[@class="book-cover"]/a/span[@class="tt"]/text()'))  # 最新章节名称
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('./div[@class="book-detail"]/dl/dd[@class="intro"]/text()'))  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./div[@class="book-cover"]/a/img/@src'))  # 书籍封面 URL
...
search_normals = ManHua2020Cartoon().search_qin_quan
