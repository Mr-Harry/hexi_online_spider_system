...
class JiSuCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.1kkk.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_info_url_pre = "http://www.1kkk.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["jisu_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["jisu_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_dict['qin_quan_platform_str'] = '极速漫画'  # 侵权平台
                ...
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('.//div[@class="desc"]/text()')).replace(' ', '')  # 书籍简介 text
...
search_normals = JiSuCartoon().search_qin_quan
