...
class katuiManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://so.77mh.co/k.php?k={}&p={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = ""
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('//*[@id="main"]/div[1]/div/div[1]/ul/dl/dd/h1/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//*[@id="main"]/div[1]/div/div[1]')[0]
                qin_quan_author_str = qin_quan_info_temp.xpath('./dl/dd/ul/li[2]/a/text()')[0]# 侵权作者
                # qin_quan_author_str = ""
                # print(qin_quan_author_str,'作者')
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./dl/dd/ul/li[1]/h1/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '卡推漫画'  # 侵权平台
...
search_normals = katuiManHuaCartoon().search_qin_quan
