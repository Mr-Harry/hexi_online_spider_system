...
class TaDuNovel:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            #'Cookie': '_ebc81c6d435065d480cb865bdbc4fec2=bf125ce5952eb80167b849426b8cc8f4668b3869215973c7533fb15b418f8a3020c4774c38a1f1e70fd6188df466d7ad; _ee965b7158f5ac6ba1d18e3e6bdf7644=8927ce3d5f23511140b678f8d884f81f; __jsluid_h=07ac00f0febaaead3bb5af580aecdf5e'
            }


    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'http://www.tadu.com/search?query={}&pageSize=30&pageNum={}'
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["tadu_search_offset"]["start"]
        _end = NOVEL_CONF["tadu_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["tadu_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["tadu_search_offset"]["start_page"] == 1:
    ...
    def get_qin_quan_id_int(self, qin_quan_url_str):
        try:
            return int(qin_quan_url_str.split('?')[0].split('/')[-1])
        except:
            return None
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//ul[@class="bookList bookBgList "]/li')
            print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('.//a[@class="authorNm"]/text()'))  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('.//a[@class="bookNm"]//text()'))  # 侵权标题
                qin_quan_url_str = "http://www.tadu.com" + "".join(n_l.xpath('./a[@class="bookImg"]/@href')).rstrip('/')  # 侵权链接
                qin_quan_id_int = self.get_qin_quan_id_int(qin_quan_url_str)  # 样本ID 数值类型
                qin_quan_mid_str = None  # 侵权ID 字符串形式
                book_type_str = "".join(n_l.xpath('.//div[@class="condition"]/span/text()')[0])
                ...
                novel_dict['qin_quan_platform_str'] = '塔读文学'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = TaDuNovel(use_proxy=True).search_novel
