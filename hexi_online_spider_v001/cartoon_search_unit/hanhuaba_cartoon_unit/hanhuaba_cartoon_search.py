...
class BiKaBiKaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.hanhuaba.com/',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://www.hanhuaba.com/plus/comic/search.php?keyword={}"  #########################################
        self.qin_quan_info_url_pre = "http://www.hanhuaba.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(url_value_to_gb2312_upper(search_key)), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="clearfix"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./p[@class="tit"]/a/@href'))  # 侵权链接
                qin_quan_author_str = ''.join(q_l.xpath('./p[@class="info"]/span')[0].xpath('./text()')).replace('作者：', '')  # 侵权作者
                qin_quan_title_str = ''.join(q_l.xpath('./p[@class="tit"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '汉化吧'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = ''.join(q_l.xpath('./p[@class="info"]/span')[2].xpath('./text()'))  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_type_str'] = ''.join(q_l.xpath('./p[@class="info"]/span')[1].xpath('./text()')).replace('分类：', '')  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                ...
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('./p[@class="intro"]/text()')).replace('\n', '')  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./a[@class="pic"]/img/@src'))  # 书籍封面 URL
