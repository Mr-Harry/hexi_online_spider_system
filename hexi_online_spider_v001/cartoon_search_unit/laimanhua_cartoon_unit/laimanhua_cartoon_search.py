...
from cartoon_search_unit.laimanhua_cartoon_unit.post_location import get_location
...
class LaiManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.laimanhua.com/e/search/result/?searchid={}&page={}"
        self.qin_quan_search_url_pre = "https://www.laimanhua.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["laimanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["laimanhua_search_offset"]["start_page"]
        # print(self.search_base_url.format(search_key))
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(get_location(search_key),_page), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('p["fl cover"]/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                try:
                    qin_quan_author_str = qin_quan_info_data.xpath('//div[@class="info"]/p/text()')[0]
                except Exception:
                    continue
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="intro_l"]/div[@class="title"]/h1/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '来漫画'  # 侵权平台
...
search_normals = LaiManHua().search_qin_quan
