...
class MuMaDongMan:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.omyschool.com/search/?keywords={}&search_by=book_name"
        self.qin_quan_search_url_pre = "https://www.omyschool.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") - CARTOON_CONF["mahua1234_offset"]["start_page"] + 1 if kwargs.get("page_num") else CARTOON_CONF["mahua1234_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="item"]/div[@class="text"]/h3[@class="title"]/a')
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="info"]/ul[@class="meta d-table"]/li/div[@class="d-cell-small value authors"]/a')[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('.//div[@class="info"]/h1[@class="heading"]/text()')).replace(' ', '').replace('\n','')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '木马动漫'  # 侵权平台
...
search_normals = MuMaDongMan().search_qin_quan
