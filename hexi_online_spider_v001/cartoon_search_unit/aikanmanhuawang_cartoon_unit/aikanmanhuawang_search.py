...
class AiKanManHuaWang:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.65dm.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_search_url_pre = "http://www.65ddm.com"
        # self.data = {
        #     'key':'{}',  #.encode("gbk")
        #
        # }
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        ...
        # print(a)
        # exit(0)
        _page = kwargs.get("page_num") + CARTOON_CONF["aikanmanhuawang_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["aikanmanhuawang_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="dmList clearfix"]/ul/li/p[@class="fl cover"]/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//div[@class='info']/p[@class='w260']/a")[0]
                ...
                qin_quan_dict['qin_quan_platform_str'] = '爱看漫画网'  # 侵权平台
...
search_normals = AiKanManHuaWang().search_qin_quan
