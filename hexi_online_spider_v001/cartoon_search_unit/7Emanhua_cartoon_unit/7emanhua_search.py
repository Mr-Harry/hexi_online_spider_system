...
class QiEManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.7edm.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_search_url_pre = "http://www.7edm.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["7emanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["7emanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="mh-search-result"]/ul[@class="mh-search-list searchrest clearfix"]/li/div[@class="mh-works-info"]/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_dict['qin_quan_platform_str'] = '7E漫画'  # 侵权平台
...
search_normals = QiEManHua().search_qin_quan
