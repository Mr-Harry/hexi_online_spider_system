...
class QiManWuCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://qiman6.com/spotlight/?keyword={}"
        self.qin_quan_info_url_pre = "http://qiman6.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") + CARTOON_CONF["bikabika_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["bikabika_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="search-result"]/div[@class="comic-list-item clearfix"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./a[@class="cover"]/@href'))  # 侵权链接
                qin_quan_author_str = ''.join(q_l.xpath('.//p[@class="comic-author"]/text()'))  # 侵权作者
                # print(qin_quan_author_str)
                if qin_quan_author_str == '暂无':
                    qin_quan_author_str = ''
                qin_quan_title_str = ''.join(q_l.xpath('.//p[@class="comic-name"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '奇漫屋'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = ''.join(q_l.xpath('./p[@class="comic-tags"]/span/text()'))  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./a[@class="cover"]/img/@src'))  # 书籍封面 URL
...
search_normals = QiManWuCartoon().search_qin_quan
