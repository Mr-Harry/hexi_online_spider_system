...
class ZhiShiHaoBa:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://manhua.zsh8.com/?s={}"
        self.qin_quan_search_url_pre = "https://manhua.zsh8.com/"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        ...
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(self.utf8_str(search_key)), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//article[contains(@class,"category-pg")]//a[@class="fusion-rollover-link"]')
            # qin_quaq_list_data = response_data.xpath('//div[(@class="fusion-rollover-content")]')
            # qin_quaq_list_data = response_data.xpath('//article[@class=""]')fusion-rollover-content
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = ''.join(q_l.xpath('./@href'))
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//table//td')[1]
                # qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('./text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '芝士豪八'  # 侵权平台
...
search_normals = ZhiShiHaoBa().search_qin_quan
if __name__ == "__main__":
    ...
    result = search_normals('天圆地方', **yangben_dict)
