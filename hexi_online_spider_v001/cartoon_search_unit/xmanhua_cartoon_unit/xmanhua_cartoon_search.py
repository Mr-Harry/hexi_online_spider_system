...
class XmanhuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.xmanhua.com/search?title=1&page=1',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://www.xmanhua.com/search?title={}&page={}"
        self.qin_quan_info_url_pre = "http://www.xmanhua.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["xmanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["xmanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="mh-list"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('.//div[@class="mh-item"]/a/@href'))  # 侵权链接
                ...
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('//p[@class="detail-info-tip"]/span')[0].xpath('./a/text()'))  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="mh-item-detali"]/h2/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = 'Xmanhua'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = ''.join(qin_quan_info_data.xpath('//p[@class="detail-info-tip"]/span')[1].xpath('./span/text()'))  # 书籍状态 （连载，完结，暂无）
                # qin_quan_dict['book_property_str'] = ''  # 书籍属性 （免费，会员，限免）
                # qin_quan_dict['author_type_str'] = ''  # 作者类型 （金牌，签约，独立 默认无）
                qin_quan_dict['book_lable_str'] = ''.join(qin_quan_info_data.xpath('//p[@class="detail-info-tip"]/span')[2].xpath('./span/text()'))  # 书籍标签 （用｜分割的字符串 ''科幻｜现实｜励志''）
                ...
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('.//div[@class="mh-item"]/a/img/@src'))  # 书籍封面 URL
...
search_normals = XmanhuaCartoon().search_qin_quan
