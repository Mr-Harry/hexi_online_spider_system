...
class DongManWu5Cartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.bikabika.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_info_url_pre = "http://www.bikabika.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["dongman5_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["dongman5_search_offset"]["start_page"]
    ...
    def re_img_url(self, url_str):
        return re.search('background-image: url\((.*?)\)', url_str).group(1)
    # 搜索视频响应解析
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('.//div[@class="mh-item-tip"]/a/@href'))  # 侵权链接
                qin_quan_author_str = ''.join(q_l.xpath('.//div[@class="mh-item-tip-detali"]/p[@class="author"]/span/a/text()'))  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="mh-item-tip-detali"]/h2[@class="title"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '动漫屋5'  # 侵权平台
                ...
                qin_quan_dict['book_cover_image_str'] = self.re_img_url(''.join(q_l.xpath('.//div[@class="mh-item-tip"]/a/p/@style')))  # 书籍封面 URL
...
search_normals = DongManWu5Cartoon().search_qin_quan
