...
class MiGuNovel:
    ...
    def search_novel(self, search_key: str, **kwargs):
        ...
        _start = NOVEL_CONF["migu_search_offset"]["start"]
        _end = NOVEL_CONF["migu_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["migu_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["migu_search_offset"]["start_page"] == 1:
                _start = kwargs.get("page_num")
                _end = kwargs.get("page_num") + 1
        for page in range(_start, _end):
            search_url = f'https://www.migu.cn/search.html?pn={page}&content={search_key}&type=reading'
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        ...
        li_list = html_str.xpath('//*[@class="list"]/ul/li')
        for li in li_list:
            novel_dict = {}
            qin_quan_author_str = "".join(li.xpath('.//div[@class="search-auth"]/text()')).replace("作者：", "")
            qin_quan_title_str = "".join(li.xpath('.//span[@class="search-title-text"]//text()'))
            qin_quan_url_str = "".join(li.xpath('./a/@href'))
            qin_quan_id_int = int(qin_quan_url_str.split('/')[-1].split('.')[0])
            ...
            novel_dict['qin_quan_platform_str'] = '咪咕阅读网'  # 侵权平台
...
search_novels = MiGuNovel(use_proxy=True).search_novel

if __name__ == '__main__':
    yangben_dict = {
        'id': '10000',
        'yang_ben_author_str': '夏末蔷薇',
        'yang_ben_title_str': '大秦',
