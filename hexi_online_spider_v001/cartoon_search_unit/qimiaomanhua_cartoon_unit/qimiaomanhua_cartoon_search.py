...
class QiMiaoManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.qimiaomh.com/action/Search?keyword={}&page={}"
        self.qin_quan_search_url_pre = "https://www.qimiaomh.com/"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["qimiaomanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["qimiaomanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="classification"]/h2/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//p[@class="author"]')[0]
                # qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者
                qin_quan_author_str = qin_quan_info_temp.xpath('./text()')[1].replace('\r','').replace('\n','').replace(' ','')  # 侵权作者
                ...
                qin_quan_dict['qin_quan_platform_str'] = '奇妙漫画'  # 侵权平台
...
search_normals = QiMiaoManHua().search_qin_quan
