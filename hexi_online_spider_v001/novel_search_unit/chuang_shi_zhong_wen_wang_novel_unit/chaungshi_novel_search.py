...
from novel_search_unit.novel_spider_tools import md5_use, get_proxy, unify_requests, str_similar,clear_text,get_date
from fake_useragent import UserAgent
from lxml import etree

class ChuangShiNovel():
    def __init__(self, use_proxy=True):
        ...
        self.search_base_url = "http://chuangshi.qq.com/search/searchindex/type/all/wd/{key_words}/category/0/updatetime/0.html?p={page}"  # 页码page从0开始

        self.novel_url_pre = "http://chuangshi.qq.com"
    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["chuang_shi_search_offset"]["start"]
        _end = NOVEL_CONF["chuang_shi_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["chuang_shi_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["chuang_shi_search_offset"]["start_page"] == 1:
        ...
        for page in range(_start, _end):
            ...
            # respose_text = unify_requests(url=self.search_base_url.format(key_words=quote(search_key), page=page),headers=self.headers,proxies=self.proxy)
            # print(self.search_base_url.format(key_words=search_key.encode('unicode_escape').decode().replace('\\u','%u'), page=page))
            respose_text = unify_requests(url=self.search_base_url.format(key_words=search_key, page=page),headers={},proxies=self.proxy)
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            book_list = select.xpath("//ul[@id='searchResultList']//li")
            for each in book_list:
                qin_quan_author_str = ''.join(each.xpath(".//b/a[@class='gray3']//text()")).replace(" ","") # 侵权作者
                qin_quan_title_str =   ''.join(each.xpath(".//h1/a[@class='gray3']//text()")).replace(" ","") # 侵权标题
                qin_quan_url_str = ''.join(each.xpath(".//h1/a[@class='gray3']/@href")).replace(" ","").split("?sword=")[0]  # 侵权链接
                qin_quan_id_int = ''.join(re.findall(r'/([0-9].*?)html',''.join(each.xpath(".//h1/a[@class='gray3']/@href")).replace(" ","")))  # 侵权ID 字符串形式

                qin_quan_mid_str = ''.join(re.findall(r'/([0-9].*?)\.html',''.join(each.xpath(".//h1/a[@class='gray3']/@href")).replace(" ","")))  # 侵权ID 字符串形式
                ...
                novel_dict['qin_quan_platform_str'] = '创世中文网'  # 侵权平台
                ...
                novel_dict['all_words_number_int'] = ''.join(each.xpath(".//div[@class='search_r_img']/p/em//text()")).replace(" ","")  # 总字数
                novel_dict['book_status_str'] = ''.join(each.xpath(".//div[@class='search_r_img']/p/i//text()")).replace(" ","")  # 书籍状态 （连载，完结，暂无）
                ...
                novel_dict['book_update_time'] = get_date(''.join(each.xpath(".//div[@class='search_r_info']//p//text()")).replace(" ",""))  # 书籍更新日期 年-月-日
                novel_dict['book_zong_zhang_jie_int'] = ''.join(each.xpath(".//div[@class='search_r_info']//p[4]//text()")).replace(" ","")  # 书籍总的章节 完结的，未完结就填目前的总章节
                # print(each.xpath(".//div[@class='search_r_info']//p//text()"))
                novel_dict['book_zong_zhang_jie_int'] = ''.join(re.findall(r'第([0-9].*?)章',novel_dict['book_zong_zhang_jie_int']))  # 书籍总的章节 完结的，未完结就填目前的总章节
                novel_dict['book_zui_xin_zhang_jie_name_str'] = ''.join(each.xpath(".//div[@class='search_r_info']//p[4]//text()")).replace(" ","").replace("最新章节：","")  # 最新章节名称
                # novel_dict['book_zui_xin_zhang_jie_name_str'] = "".join(re.findall(r'章(.*?) \(',novel_dict['book_zui_xin_zhang_jie_name_str']))
                novel_dict['book_introduce_text'] = ''.join(each.xpath(".//p[@class='search_i']/em[@class='gray']//text()")).replace(" ","")  # 书籍简介 text
                novel_dict['book_cover_image_str'] = "http:"+''.join(each.xpath(".//div[@class='search_r_img']/a/img/@src")).replace(" ","")  # 书籍封面 URL
                novel_dict['book_detail_url_str'] = ''.join(each.xpath(".//div[@class='search_r_img']/a/@href")).replace(" ","")  # 书籍详情URL
                novel_dict['book_detail_id_int'] = qin_quan_id_int  # 书籍详情ID 数字形式
                novel_dict['book_detail_id_str'] = qin_quan_mid_str  # 书籍详情ID 字符形式
                ...
                novel_dict['author_page_url_str'] = ''.join(each.xpath(".//b/a[@class='gray3']/@href")).replace(" ","")  # 作者主页链接
...
# 统一的调用 search_novels
search_novels = ChuangShiNovel(use_proxy=True).search_novel
if __name__ == "__main__":
    str1 = "我的"

    print(str1.encode('unicode_escape').decode().replace('\\u','%u'))
    # exit(0)
    yangben_dict = {
        'id': '10000',
    ...
    result = search_novels('我的', **yangben_dict)
