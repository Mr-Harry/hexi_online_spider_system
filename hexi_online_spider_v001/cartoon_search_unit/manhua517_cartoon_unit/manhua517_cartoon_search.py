...
class ManHua517Cartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.517mh.net/sort/?key={}&page={}"
        self.qin_quan_info_url_pre = "http://www.517mh.net"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhua517_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhua517_search_offset"]["start_page"]
    ...
    def re_img_url(self, url_str):
        print(url_str)
        return re.search('background: url\(\"(.*?)\"', url_str).group(1)
    # 搜索视频响应解析
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="comic-list col5 clearfix"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./div[@class="info"]/h3[@class="title"]/a/@href'))  # 侵权链接
                qin_quan_author_str = ''.join(q_l.xpath('./div[@class="info"]/p[@class="desc"]/text()')).replace('连载中', '').replace('已完结', '').replace(' ', '')  # 侵权作者
                qin_quan_title_str = ''.join(q_l.xpath('./div[@class="info"]/h3[@class="title"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '517漫画网'  # 侵权平台
                ...
                book_status_str = ''.join(q_l.xpath('./div[@class="info"]/p[@class="desc"]/text()'))
                qin_quan_dict['book_status_str'] = '连载中' if '连载中' in book_status_str else ('已完结' if '已完结' in book_status_str else '') # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./a/img/@data-src'))  # 书籍封面 URL
...
search_normals = ManHua517Cartoon().search_qin_quan
