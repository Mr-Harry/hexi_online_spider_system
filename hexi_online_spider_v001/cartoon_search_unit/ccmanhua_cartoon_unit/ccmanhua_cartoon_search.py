...
class CcManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "http://www.ccmh15.com/search/result/keyword/{}"

        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "http://www.ccmh15.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        # 如果需要翻页 设置页数
        _page = kwargs.get("page_num") + CARTOON_CONF["ccmanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["douding_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            # qin_quaq_list_data = response_data.xpath('//*[@id="dmList"]/ul/li/dl/dt/a')
            qin_quaq_list_data = response_data.xpath('//div[@class="dmList clearfix"]/ul/li')
            # print(len(qin_quaq_list_data))
            # 循环列表
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./dl/dt/a/@href'))  # 侵权链接
                ...
                try:
                    # 请求详情页
                    # response_data = json.loads(response.text)
                    # li= self.qin_quan_info(qin_quan_url_str).text
                    # print(li)
                    data1 = self.qin_quan_info(qin_quan_url_str)
                    data1.encoding = data1.apparent_encoding
                    # print(data1)
                    qin_quan_info_data = etree.HTML(data1.text)
                    # print(qin_quan_info_data)
                    # print(etree.tostring(qin_quan_info_data))
                    # print(response_dict)

                except:
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="intro_l"]')[0]
                # print(qin_quan_info_temp)
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('//div[@class="info"]/p[2]/a/text()'))# 侵权作者
                # print(qin_quan_author_str,"90"*10)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[1]/h1/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = 'cc漫画'  # 侵权平台
                ...
                # qin_quan_dict['book_dian_ji_number_int'] = ''.join(qin_quan_info_temp.xpath('//*[@id="hits_span"]/text()'))  # 作品点击量
                # print(qin_quan_dict['book_dian_ji_number_int'])
                ...
                # print(qin_quan_info_temp.xpath('./div[2]/p')[6],"xyy")
                # qin_quan_dict['book_status_str'] = ''.join(qin_quan_info_temp.xpath('./div[2]/p')[6].xpath('./text()'))  # 书籍状态 （连载，完结，暂无）
...
search_normals = CcManHuaCartoon().search_qin_quan
if __name__ == "__main__":
    yangben_dict = {
        ...
        'page_num': 1,
        # 'id': '10000',
        # 'yang_ben_author_str': '春山モト',
        # 'yang_ben_title_str': '相信后辈是个小可爱的我真是个笨蛋',
        # 'yang_ben_url_str': 'http://www.ccmh15.com/manhua/24873/',
        # 'page_num': 1,
    }
