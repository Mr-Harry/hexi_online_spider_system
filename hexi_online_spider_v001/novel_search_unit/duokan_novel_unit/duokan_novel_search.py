...
class DuoKanNovel():
    def __init__(self, use_proxy=True):
        ...
        self.search_base_url = "https://www.duokan.com/search/{key_words}/{page}"  # 页码page从0开始

        self.novel_url_pre = "https://www.duokan.com"
    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["duo_kan_search_offset"]["start"]
        _end = NOVEL_CONF["duo_kan_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["duo_kan_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["duo_kan_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            book_list = select.xpath("//ul[@class='u-list j-list']//li")
            for each in book_list:
                qin_quan_author_str = ''.join(each.xpath(".//div[@class='u-author']/span//text()")).replace(" ","") # 侵权作者
                qin_quan_title_str =   ''.join(each.xpath(".//div[@class='wrap']/a//text()")).replace(" ","") # 侵权标题
                qin_quan_url_str = self.novel_url_pre+''.join(each.xpath(".//div[@class='wrap']/a/@href")).replace(" ","")  # 侵权链接
                qin_quan_id_int = ""# 侵权ID 数值类型

                qin_quan_mid_str = ''.join(each.xpath(".//div[@class='wrap']/a/@href")).replace("/book/","")   # 侵权ID 字符串形式
                ...
                novel_dict['qin_quan_platform_str'] = '多看阅读'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = DuoKanNovel(use_proxy=True).search_novel
