...
class KuManWu:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.kuman55.com/search.php?keyword={}"
        self.qin_quan_search_url_pre = "http://www.kuman55.com/"
    #转意函数
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="mh-item-detali"]/h2["title"]/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//div["info"]/p[@class="subtitle"]')[0]
                # qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者
                qin_quan_author_str = qin_quan_info_temp.xpath('./text()') [0][3:]  # 侵权作者
                ...
                qin_quan_dict['qin_quan_platform_str'] = '酷漫屋'  # 侵权平台
...
search_normals = KuManWu().search_qin_quan
if __name__ == "__main__":
    ...
    result = search_normals('开', **yangben_dict)
