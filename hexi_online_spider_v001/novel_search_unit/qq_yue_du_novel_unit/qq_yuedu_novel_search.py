...
class QQYueDuNovel():
    def __init__(self, use_proxy=True):
        ...
        self.search_base_url = "http://yunqi.qq.com/search/searchindex/type/all/wd/{key_words}.html?snum=10&p={page}"  # 页码page从0开始

        self.novel_url_pre = "yunqi.qq.com"
    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["qq_yue_du_search_offset"]["start"]
        _end = NOVEL_CONF["qq_yue_du_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["qq_yue_du_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["qq_yue_du_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            book_list = select.xpath("//div[@id='search_result']//div[@class='book']")
            for each in book_list:
                qin_quan_author_str = ''.join(each.xpath(".//dd[@class='auth']/a//text()")).replace(" ","") # 侵权作者
                qin_quan_title_str =   ''.join(each.xpath(".//h3/a//text()")).replace(" ","") # 侵权标题
                qin_quan_url_str = ''.join(each.xpath(".//h3/a/@href")).replace(" ","")  # 侵权链接
                # print(qin_quan_url_str)
                qin_quan_id_int = "".join(re.findall(r'/([0-9].*?)\.html',qin_quan_url_str))# 侵权ID 数值类型

                qin_quan_mid_str = "".join(re.findall(r'/([0-9].*?)\.html',qin_quan_url_str))  # 侵权ID 字符串形式
                ...
                novel_dict['qin_quan_platform_str'] = 'qq阅读'  # 侵权平台
                ...
                novel_dict['all_words_number_int'] = "".join(each.xpath(".//div[@class='book_info']//dl[2]//dd[3]//text()")).replace(" ","")  # 总字数
                novel_dict['book_shou_cang_number_int'] = "".join(each.xpath(".//div[@class='book_info']//dl[2]//dd[@class='auth']//text()")).replace(" ","")  # 总收藏数
                novel_dict['book_status_str'] = "".join(each.xpath(".//div[@class='book_info']//dl[1]//dd[@class='w_auth'][2]//text()")).replace(" ","")  # 书籍状态 （连载，完结，暂无）
...
# 统一的调用 search_novels
search_novels = QQYueDuNovel(use_proxy=True).search_novel
