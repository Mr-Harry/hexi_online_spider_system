...
class LiuManHUa:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://m.sixmh6.com/search?keyword={}"
        self.qin_quan_search_url_pre = "http://m.sixmh6.com"
    #转意函数
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="result-list"]/a')
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./@href'))
                qin_quan_author_str = ''.join(q_l.xpath('./li/div/p/text()')[0])  # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('./li/div/h2/text()')).replace('\xa0','')  # 侵权标题
                ...
                # qin_quan_info_temp = qin_quan_info_data.xpath("//div[@class='MidL1_L']/div[@class='txt']")[0]
                # qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者
                # if not qin_quan_author_str:
                #     continue
                # qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="MidL1_L"]/div[@class="tit"]/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '6漫画'  # 侵权平台
...
search_normals = LiuManHUa().search_qin_quan
