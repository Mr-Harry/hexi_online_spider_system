...
class kuaikanManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://www.kuaikanmanhua.com/s/result/{}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "https://www.kuaikanmanhua.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") + CARTOON_CONF["kuaikanmanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["kuaikanmanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('//*[@id="__layout"]/div/div/div[3]/div/div/div/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//*[@id="__layout"]/div/div/div[2]/div/div[1]/div[1]')[0]
                qin_quan_author_str = qin_quan_info_temp.xpath('./div[2]/div[1]/text()')[0]# 侵权作者
                # print(qin_quan_author_str,'作者')
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[2]/h3/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '快看漫画'  # 侵权平台
...
search_normals = kuaikanManHuaCartoon().search_qin_quan
