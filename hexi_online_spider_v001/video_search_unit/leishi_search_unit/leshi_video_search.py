...
class LeShiVideo:
    ...
    def __init__(self, use_proxy=True):
        self.search_video_url = "http://search.lekan.letv.com/lekan/apisearch_json.so?from=pc&jf=1&hl=1&dt=1,2&ph=420001,420002&show=4&pn={1}&ps=25&wd={0}"
        self.headers = {
    ...
    def parse_search_video_song(self, response) -> list:
        ...
        try:
            ...
            result_list = []
            try:
                video_song_list_data = tree.get('data_list', [])
            except:
                video_song_list_data =[]
            for v_s in video_song_list_data:
                video_dict = dict()
                video_dict["video2_title"] = v_s.get("name", '').replace('\x01', '').replace('\x02', '')
...
search_songs = LeShiVideo(use_proxy=True).search_songs
if __name__ == '__main__':
    ...
    print(LeShiVideo().search_songs('班淑传奇', **kwags))
