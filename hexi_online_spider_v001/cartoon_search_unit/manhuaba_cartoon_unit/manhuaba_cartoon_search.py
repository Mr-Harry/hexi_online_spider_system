...
class AitaoManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # self.search_base_url = "http://m.kanbook.net/search/{}token=53200c24a32395641d66236a663a87fa&page={}"
        # url模板
        self.search_base_url = "https://www.manhuaba.cc/statics/search.aspx?key={}&page={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "https://www.manhuaba.cc/"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhuaba_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhuaba_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('/html/body/div[4]/div[2]/div/div/div/ul')
            # print(qin_quaq_list_data)
            # print(len(qin_quaq_list_data))
            # 循环列表
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./li[2]/a/@href')) # 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//div[@id="intro_l"]')[0]
                qin_quan_author_str = qin_quan_info_temp.xpath('.//div[4]/span[1]/a/text()')[0]# 侵权作者
                # print(qin_quan_author_str,'作者')
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[3]/h1/text()')).strip()#  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '漫画吧'  # 侵权平台
...
search_normals = AitaoManHuaCartoon().search_qin_quan
