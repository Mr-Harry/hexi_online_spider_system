...
class DouDingCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.docin.com/touch_comic/search.do?rkey=1',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://www.docin.com/touch_comic/search_more.do?pageNo={}&rkey={}"
        self.qin_quan_search_url_pre = "http://www.docin.com"
        self.qin_quan_info_url_base = "https://manhua.docin.com/comic-{}.html"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["douding_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["douding_search_offset"]["start_page"]
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(_page, search_key), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//li/div[@class="p clear"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = ''.join(q_l.xpath('./a/@href'))  # 侵权链接
                qin_quan_id = get_parms_value(qin_quan_url_str, 'comic_id')
                qin_quan_url_str = self.qin_quan_info_url_base.format(qin_quan_id)
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('.//div[@class="mh-bd"]//div[@class="detail-zone clear"]')[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('//ul[@class="info-list"]/li[@class="st1"]/p[@class="txt-elip"]/a/text()')).replace('作者：', '').replace(' ', '')  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="p-title"]/text()')).replace(' ', '')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '豆丁漫画'  # 侵权平台
                ...
                qin_quan_dict['all_read_int'] = ''.join(qin_quan_info_data.xpath('//ul[@class="info-list"]/li[@class="st1"]')[0].xpath('./p')[1].xpath('./span/text()'))  # 总阅读数 int
                ...
                qin_quan_dict['book_shou_cang_number_int'] = ''.join(qin_quan_info_data.xpath('//ul[@class="info-list"]/li[@class="st1"]')[1].xpath('./p')[0].xpath('./span/text()')).replace(',', '')  # 作品收藏量
                book_status_str = ''.join(q_l.xpath('.//div[@class="update-info"]/text()'))
                qin_quan_dict['book_status_str'] = '完结' if '完结' in book_status_str else '连载中'  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_update_time'] = ''.join(qin_quan_info_data.xpath('.//span[@class="update-time"]/text()')).replace('更新时间：', '').replace('年', '-').replace('月', '-').replace('日', '')  # 书籍更新日期 年-月-日
                # qin_quan_dict['book_zong_zhang_jie_int'] = ''  # 书籍总的章节 完结的，未完结就填目前的总章节
                # qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = ''  # 最新章节名称
                qin_quan_dict['book_introduce_text'] = ''.join(qin_quan_info_temp.xpath('.//p[@class="info-desc"]/text()')).replace('\r', '').replace('\n', '')  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(qin_quan_info_temp.xpath('.//div[@class="detail-img"]/a/img/@data-style'))  # 书籍封面 URL
...
search_normals = DouDingCartoon().search_qin_quan
if __name__ == "__main__":
    ...
    result = search_normals('正太', **yangben_dict)
