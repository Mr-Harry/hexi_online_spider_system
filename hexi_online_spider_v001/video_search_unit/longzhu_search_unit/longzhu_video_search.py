...
class LongZhuVideo:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            'Origin': 'https://search.longzhu.com',
            'Sec-Fetch-Site': 'same-site',
            'Sec-Fetch-Mode': 'cors',
            'Sec-Fetch-Dest': 'empty',
            'Referer': 'https://search.longzhu.com/',
            'Accept-Language': 'zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7',
        }
        self.api_video_search_key = ''
        self.yangben_dict = {}
        self.video_search_live_base_url = "https://searchapi.longzhu.com/api/v2/search/room?key=%s&pageIndex={}&pageSize=%s"  # 页码page从0开始
        self.video_search_video_base_url = "http://searchapi.longzhu.com/api/search/videoreplay?key=%s&pageIndex={}&pageSize=%s"  # 页码page从0开始

    def search_video(self, search_key: str, video_search_type="",
                     start_page=config.get("longzhu_video_search_offset", {}).get("start", 0),
                     end_page=config.get("longzhu_video_search_offset", {}).get("end", 1),
                     page_size=config.get("longzhu_video_search_offset", {}).get("pagesize", 20),
                     yangben_dict=None):
        search_result = []
        if not search_key:
        ...
        if (0 in video_search_type_list) or (5 in video_search_type_list):
            ...
            for i in range(start_page, end_page):
                ...
                search_result.append(self.parse_search_live(search_response))

        # 视频搜索
        if (0 in video_search_type_list) or (2 in video_search_type_list):
            video_url_temp = self.video_search_video_base_url % (search_key, page_size)
            for i in range(start_page, end_page):
                video_url = video_url_temp.format(i)
                # print(video_url)
                search_response = self.get_response_single(video_url)
    ...
    # 搜索直播响应解析
    def parse_search_live(self, response) -> list:
        try:
            ...
            result_list = []
            video_list_data = response_dict.get('data', {}).get('items', []) if response_dict.get('data', {}) else []
            for v_i in video_list_data:
                video_dict = dict()
                video_dict["video2_title"] = v_i.get('live', {}).get('title','')
                if not check_video_title(video_dict.get("video2_title"),self.api_video_search_key):
                    continue
                video_dict["video2_url"] = "http://star.longzhu.com/" + str(v_i.get('live', {}).get('url', ''))
                video_dict["video2_author"] = v_i.get('name', '')
                video_dict["video2_url_hash"] = md5_use(video_dict.get("video2_url"))
                video_dict["video2_platform"] = "龙珠"
    ...
    def parse_search_video(self, response) -> list:
        try:
            ...
            for v_i in video_list_data:
                video_dict = dict()
                video_dict["video2_title"] = v_i.get('title', '')
                if not check_video_title(video_dict.get("video2_title"), self.api_video_search_key):
                    continue
                video_dict["video2_url"] = "http://replay.longzhu.com/v/" + str(v_i.get('videoId', ''))
                video_dict["video2_author"] = v_i.get('userName', '')
                video_dict["video2_url_hash"] = md5_use(video_dict.get("video2_url"))
                video_dict["video2_platform"] = "龙珠"
                video_dict["video2_type"] = 2
                # 样本数据
                if self.yangben_dict and isinstance(self.yangben_dict, dict):
                    video_dict["video_title"] = self.yangben_dict.get("video_title")
                    video_dict["video_author"] = self.yangben_dict.get("video_author")
...
search_songs = LongZhuVideo(use_proxy=True).search_video


if __name__ == "__main__":
    longzhu = LongZhuVideo(use_proxy=True)
    result = longzhu.search_video('我')
    print(len(result))
    print(result)