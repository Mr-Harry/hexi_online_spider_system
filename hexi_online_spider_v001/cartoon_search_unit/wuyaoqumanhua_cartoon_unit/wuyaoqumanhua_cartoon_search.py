...
class woyqoqvCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "http://www.517manhua.com/statics/search.aspx?key={}&page={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "http://www.517manhua.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["woyaoqu_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["woyaoqu_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('/html/body/div[2]/div[1]/div/div[2]/ul/li')
            # print(qin_quaq_list_data)
            # print(len(qin_quaq_list_data))
            # 循环列表
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./div[2]/div/h4/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('/html/body/div[3]/div[2]')[0]
                ...
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[1]/h4/a/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '我要去'  # 侵权平台
...
search_normals = woyqoqvCartoon().search_qin_quan
