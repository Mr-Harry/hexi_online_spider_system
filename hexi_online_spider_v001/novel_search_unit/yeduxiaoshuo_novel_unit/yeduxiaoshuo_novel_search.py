...
class YeDuXiaoShuo:
    ...
    def get_author(self, author_url):
        ...
        qin_quan_author_str = ''.join(html_str.xpath('//a[@class="author fl"]/text()'))
        return qin_quan_author_str
    #侵权标题
    def get_title(self, book_url):
        response = unify_requests(url=book_url, headers=self.headers).text
        html_str = etree.HTML(response)
        qin_quan_title_str = ''.join(html_str.xpath('//span[@class="name fl"]/text()'))
    ...
    def search_novel(self, search_key: str, **kwargs):
        ...
        _start = NOVEL_CONF["yeduxiaoshuo_search_offset"]["start"]
        _end = NOVEL_CONF["yeduxiaoshuo_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["yeduxiaoshuo_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["yeduxiaoshuo_search_offset"]["start_page"] == 1:
                _start = kwargs.get("page_num")
                _end = kwargs.get("page_num") + 1
        for page in range(_start, _end):
            search_url = f'http://huoshenwx.com/modules/article/search.php'
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        ...
        # div_list = html_str.xpath('//a[@class="bigpic-book-name"]/@href')
        # div_list = response
        author_url = response
        # print(len(div_list))
        # print(response.text)
        # for div in div_list:
        #     try:
        #         author_url = response['url_articleinfo']
        #     except:
        #         author_url = None
        if author_url:
        ...
        novel_dict['qin_quan_platform_str'] = '夜读小说'  # 侵权平台
...
search_novels = YeDuXiaoShuo(use_proxy=True).search_novel
