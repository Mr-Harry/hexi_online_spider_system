...
from video_search_unit.kuai_shou_search_unit.kuai_shou_service import KuaiShouService
...
class KuaiShou():
    ...
    def search_songs(self, song_name='', **kwargs) -> list:
        ...
        # 搜索页码search_page
        _start = config["kauishou_video_search_offset"]["start_page"]
    ...
    def get_response_single(self, song_name, search_page,_type):
        kw = {"keyword":song_name,
              "offset":search_page,
              "proxise":self.proxy}
        return KuaiShouService().judeg_type_back(_type,**kw)
    ...
    def parse_search_video_song(self, response_json) -> list:
        ...
        for each in response_json["feeds"]:
            # print(each)
            video_dict = dict()
            # info_id = str(each["share_info"].split("userId")[0])
            info_id = str(each["share_info"].split("photoId=")[-1])
            # video_duration =
            if "duration" in each:
                video_dict["video2_title"] = each["caption"]
                video_dict["video2_url"] = "https://video.kuaishou.com/featured/{}".format(info_id)
                video_dict["video2_author"] = each["user_name"]
                video_dict["video2_pubtime"] = each["time"]
                video_dict["video2_url_hash"] = md5_use(video_dict.get("video2_url"))
                video_dict["video2_platform"] = "快手"

                duration_str_temp = each["duration"]/1000
                video_dict["video2_id"] = info_id
                # print(each)
                ...
                # print("adsfadsfasdfasdfasdf",video_dict)
            # else:
                # print("xxxxxxxxxxxxxxxxx",each)
        return result_list
...
search_songs = KuaiShou(use_proxy=True).search_songs
