...
# url = "http://www.fbook.net/Search/Index?searchType=All&searchKey=1"
#
# payload = {}
# headers = {
...
class TianXiaShuMengNovel:
    ...
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'http://www.fbook.net/Search/Index?searchType=All&searchKey={}&page={}'
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["tianxia_shumeng_search_offset"]["start"]
        _end = NOVEL_CONF["tianxia_shumeng_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["tianxia_shumeng_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["tianxia_shumeng_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//div[@class="litab_1 clearfix"]')
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('./div/h4/text()')).replace('作者：', '')  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('./div/h3/a/strong/text()'))  # 侵权标题
                qin_quan_url_str = "".join(n_l.xpath('./div/h3/a/@href')) # 侵权链接
                ...
                novel_dict['qin_quan_platform_str'] = '天下书盟'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = TianXiaShuMengNovel(use_proxy=True).search_novel
