...
class XiuShiBaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.xiushiba.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_info_url_pre = "https://www.xiushiba.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["xiushiba_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["xiushiba_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="clearfix"]/div[@class="details-info-min col-md-12 col-sm-12 col-xs-12 clearfix news-box-txt p-0"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('.//div[@class="col-md-9 col-sm-8 col-xs-9 clearfix pb-0"]//ul[@class="info clearfix"]/li')[0].xpath('./a/@href'))  # 侵权链接
                qin_quan_author_str = ''.join(q_l.xpath('.//div[@class="col-md-9 col-sm-8 col-xs-9 clearfix pb-0"]//ul[@class="info clearfix"]/li')[4].xpath('./text()'))  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="col-md-9 col-sm-8 col-xs-9 clearfix pb-0"]//ul[@class="info clearfix"]/li')[0].xpath('./a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '秀十八漫画'  # 侵权平台
                ...
                qin_quan_dict['book_type_str'] = '|'.join(q_l.xpath('.//div[@class="col-md-9 col-sm-8 col-xs-9 clearfix pb-0"]//ul[@class="info clearfix"]/li')[3].xpath('./a/text()'))  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                ...
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('.//div[@class="col-md-9 col-sm-8 col-xs-9 clearfix pb-0"]//ul[@class="info clearfix"]/li')[7].xpath('./span[@class="details-content-default"]/text()'))  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('.//div[@class="col-md-3 col-sm-4 col-xs-3 news-box-txt-l clearfix"]/a/@data-original'))  # 书籍封面 URL
...
search_normals = XiuShiBaCartoon().search_qin_quan
if __name__ == "__main__":
    ...
    result = search_normals('爱', **yangben_dict)
