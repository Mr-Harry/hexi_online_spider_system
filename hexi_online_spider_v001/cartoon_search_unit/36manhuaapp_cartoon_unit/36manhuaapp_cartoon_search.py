...
class BiKaBiKaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://soumh.leeyegy.com/search.aspx?key={}&page={}&siteid=manhuaapp2"
        self.qin_quan_info_url_pre = "https://infosmanhua.pysmei.com/BookFiles/Html/1/{}/info.html"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhua36app_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhua36app_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # qin_quaq_list_data = response_data.xpath('//ul[@class="mh-list col7"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre.format(q_l.get('Id', ''))  # 侵权链接
                qin_quan_author_str = q_l.get('Author', '')  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = q_l.get('Name', '')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '36漫画app'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = q_l.get('BookStatus', '')  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_type_str'] = q_l.get('CName', '')  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                # qin_quan_dict['book_update_time'] = ''  # 书籍更新日期 年-月-日
                # qin_quan_dict['book_zong_zhang_jie_int'] = ''  # 书籍总的章节 完结的，未完结就填目前的总章节
                qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = q_l.get('LastChapter', '')  # 最新章节名称
                qin_quan_dict['book_introduce_text'] = q_l.get('Desc', '')  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = q_l.get('Img', '')  # 书籍封面 URL
