...
class ZongHengNovel:
    ...
    def search_novel(self, search_key: str, **kwargs):
        ...
        _start = NOVEL_CONF["zhangyue_search_offset"]["start"]
        _end = NOVEL_CONF["zhangyue_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["zhangyue_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["zhangyue_search_offset"]["start_page"] == 1:
                _start = kwargs.get("page_num")
                _end = kwargs.get("page_num") + 1
        for page in range(_start, _end):
            search_url = f'http://www.ireader.com/index.php?ca=search.index&pca=search.index&keyword={search_key}&page={page}'
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        ...
        li_list = html_str.xpath('//ul[@class="sResult"]/li')
        for li in li_list:
            novel_dict = {}
            qin_quan_url_str = "".join(li.xpath('./div[@class="cf"]/a/@href'))
            if 'http' not in qin_quan_url_str:
                qin_quan_url_str = 'http://www.ireader.com/' + qin_quan_url_str
            else:
                qin_quan_url_str = qin_quan_url_str
            qin_quan_title_str = "".join(li.xpath('.//a/h3/text()')).strip()
            qin_quan_id_int = int(qin_quan_url_str.split("bid=")[-1])
            qin_quan_mid_str = qin_quan_url_str.split("bid=")[-1]
            qin_quan_author_str = "".join(li.xpath('.//p[@class="author"]/text()')).strip().replace('作者：','')
            ...
            novel_dict['yang_ben_batch_id_int'] = kwargs.get('yang_ben_batch_id_int', '')  # 样本批次ID
            novel_dict['book_type_str'] = None

            novel_dict['search_key_words_str'] = kwargs.get('search_key_words_str', '')  # 搜索关键词
            novel_dict['qin_quan_platform_str'] = '掌阅'  # 侵权平台
