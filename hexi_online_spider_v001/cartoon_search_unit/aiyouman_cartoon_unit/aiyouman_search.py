...
class AiYouManCartoon:
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") - CARTOON_CONF["aiyouman_search_offset"]["start_page"] + 1 if kwargs.get("page_num") else CARTOON_CONF["aiyouman_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # qin_quaq_list_data = response_data.xpath('//div[@id="J_comicListBox"]/li')
            qin_quaq_list_data = response_data['data']
            # qin_quaq_list_data = response_data.xpath('//body/text()')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    qin_quan_info_data = q_l
                    # qin_quan_info_data = etree.HTML(self.qin_quan_info(qin_quan_url_str).text)
                    # print(etree.tostring(qin_quan_info_data))
                    # print(response_dict)
                except:
                    continue
                # qin_quan_info_temp = qin_quan_info_data.xpath('.//div[@class="product"]')[0]
                # qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('.//div[@id="about_kit"]/ul/li')[1].xpath('./text()'))  # 侵权作者
                qin_quan_author_str = qin_quan_info_data['comic_author']   #侵权作者
                if not qin_quan_author_str:
                    continue
                # qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="p-title"]/text()')).replace(' ', '')  # 侵权标题
                qin_quan_title_str = qin_quan_info_data['comic_name']
                ...
                qin_quan_dict['qin_quan_url_hash_str'] = str(yangben_id) + '|' + md5_use(
                    response.url)  # 唯一索引，样本task_id 侵权url（md5）
...
search_normals = AiYouManCartoon().search_qin_quan
if __name__ == "__main__":
    ...
    result = search_normals('李', **yangben_dict)
