...
class JiuJiu5ManHUa:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://m.99wushu.com/statics/search.aspx?key={}"
        self.qin_quan_search_url_pre = "http://m.99wushu.com"
    #转意函数
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="imgBox"]/ul/li/a[@class="ImgA autoHeight"]')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//div[@class='sub_r']/p[@class='txtItme']")[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))[3:]  # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath("//div[@class='sub_r']/p[@class='txtItme h1']/text()"))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '久久5叔漫画'  # 侵权平台
...
search_normals = JiuJiu5ManHUa().search_qin_quan
