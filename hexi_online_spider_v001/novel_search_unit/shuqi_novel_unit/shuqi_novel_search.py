...
class ShuQiNovel:
    def __init__(self, use_proxy=True):
        ...
        self.search_base_url = "https://read.xiaoshuo1-sm.com/novel/i.php?do=is_search&isNewBind=0&tk=&imei=&sn=&channelId" \
                          "=&clientPatch=&clientVersion=&device=&deviceId=&model=&osName=&osVersion=&umidToken" \
                          "=&appVersion=&app=&platform=3&ykToken=&ykuid=&shuqi_h5=&token=eyJhbGciOiJIUzUxMiJ9" \
                          ".eyJzdWIiOiI4MDAwMDAwIiwidXRkaWQiOiIiLCJpbWVpIjoiIiwic24iOiIiLCJleHAiOjE2MDA2OTcwMTYsInVzZXJJZCI6IjgwMDAwMDAiLCJpYXQiOjE2MDA2NzkwMTYsIm9haWQiOiIiLCJwbGF0Zm9ybSI6IjAifQ.Y7nkOKIXNhe8NK9ajnAvvZyKOP0UoJJBtvk5-YCgQADzUF7ZNf7CoTzbbdMnPHHX0yfFuHUmqtPufmdsjJ-UNA&user_id=8000000&q={}&page={}&size=10&filterMigu=1&ver=&p=3&timestamp=1600679156&sign=24291c5ec1bf18b5e2f1f6adee5552f6 "
    ...
    def search_novel(self, search_key: str, **kwargs):
        ...
        _start = NOVEL_CONF["shuqi_search_offset"]["start"]
        _end = NOVEL_CONF["shuqi_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["shuqi_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["shuqi_search_offset"]["start_page"] == 1:
        ...
        for page in range(_start, _end):
            ...
            respose_text = unify_requests(url=self.search_base_url.format(search_key, page), headers=self.headers, proxies=self.proxy)
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_dict.get('data', []) if response_dict.get('data', []) else []
            json_novel_first = response_dict.get('aladdin', {})
            if json_novel_first:
                novel_list_data.append(json_novel_first)
...
# 统一的调用 search_novels
search_novels = ShuQiNovel(use_proxy=True).search_novel
