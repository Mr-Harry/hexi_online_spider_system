...
class DongManWuCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Referer': 'https://www.imhwu.com/',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "https://www.imhwu.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_info_url_pre = "http://www.imhwu.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["dongmanwu_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["dongmanwu_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="mh-search-list searchrest clearfix"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('.//div[@class="mh-works-title clearfix"]/h4/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('.//p[@class="mh-pdt30 works-info-tc"]/span[@class="one"]/em/a/text()'))  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="mh-works-title clearfix"]/h4/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画屋'  # 侵权平台
                ...
                if qin_quan_dict["title_similar_number_float"] >= CARTOON_CONF["qin_quan_similar"] and qin_quan_dict[
                    ...
                    qin_quan_dict["qin_quan_flag_int"] = 1
                qin_quan_dict['all_recommend_str'] = ''.join(q_l.xpath('.//p[@class="mh-works-tags"]/span')[1].xpath('./em/text()')) + '点赞'  # 总推荐数 str
                ...
                qin_quan_dict['book_shou_cang_number_int'] = ''.join(q_l.xpath('.//p[@class="mh-works-tags"]/span')[0].xpath('./em/text()'))  # 作品收藏量
                ...
                qin_quan_dict['book_lable_str'] = ''.join(q_l.xpath('.//p[@class="mh-works-tags"]/a/text()'))  # 书籍标签 （用｜分割的字符串 ''科幻｜现实｜励志''）
                # qin_quan_dict['book_type_str'] = ''  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                qin_quan_dict['book_update_time'] = ''.join(q_l.xpath('.//p[@class="mh-up-time fr"]/text()')).replace('最后更新时间：', '')  # 书籍更新日期 年-月-日
                # qin_quan_dict['book_zong_zhang_jie_int'] = ''  # 书籍总的章节 完结的，未完结就填目前的总章节
                # qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = ''  # 最新章节名称
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('.//p[@class="mh-works-decs"]/text()'))  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('.//div[@class="mh-nlook-w"]/a/img/@src'))  # 书籍封面 URL
...
search_normals = DongManWuCartoon().search_qin_quan
