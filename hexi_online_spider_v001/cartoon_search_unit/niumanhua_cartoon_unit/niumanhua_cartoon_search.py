...
class niuManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://www.manhuaniu.com/search/?keywords={}&page={}"

        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "https://www.manhuaniu.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        # 如果需要翻页 设置页数
        _page = kwargs.get("page_num") + CARTOON_CONF["niumanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["niumanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//*[@id="contList"]/li/p/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('/html/body/div[4]/div/div/div[2]/div[1]/div[1]/div[2]')[0]
                # print(qin_quan_info_temp)
                qin_quan_author_str1 = ''.join(qin_quan_info_temp.xpath('./ul/li[2]/span[2]/text()'))# 侵权作者
                qin_quan_author_str = qin_quan_author_str1.replace("\n                        \n                       ", "")
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[1]/h1/span/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '漫画牛'  # 侵权平台
...
search_normals = niuManHuaCartoon().search_qin_quan
