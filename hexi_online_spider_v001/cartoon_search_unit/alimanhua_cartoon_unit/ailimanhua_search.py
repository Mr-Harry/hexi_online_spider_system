...
class ALiManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.alimanhua.com/e/search/result/index.php?page={}&searchid=504"
        self.qin_quan_search_url_pre = "http://www.alimanhua.com"
        # self.data = {
        #     'key':'{}',  #.encode("gbk")
        #
        # }
    ...
    def utf8_str(self,num):
        ...
        # str_u = num.encode("gbk")
        return str_u
    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        # a = search_key.encode('gbk')
        # print(a)
        # exit(0)
        _page = kwargs.get("page_num") + CARTOON_CONF["alimanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["alimanhua_search_offset"]["start_page"]
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(_page), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="dmList clearfix"]/ul/li/p/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//div[@class='detailInfo']/ul/li")[1]
                ...
                qin_quan_dict['qin_quan_platform_str'] = '阿里漫画'  # 侵权平台
...
search_normals = ALiManHua().search_qin_quan
