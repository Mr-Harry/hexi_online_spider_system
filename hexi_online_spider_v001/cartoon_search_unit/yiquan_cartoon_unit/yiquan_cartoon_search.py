...
class YiQuanCartoon:
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="cComicList"]/li')
            ...
            for q_l in qin_quaq_list_data:
                ...
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('./a/text()')).replace(' ', '')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '一拳漫画'  # 侵权平台
...
search_normals = YiQuanCartoon().search_qin_quan
