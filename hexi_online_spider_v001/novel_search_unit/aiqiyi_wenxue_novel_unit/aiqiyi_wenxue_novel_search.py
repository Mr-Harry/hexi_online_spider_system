...
class ZhuiShuShenQiNovel:
    ...
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'http://wenxue.iqiyi.com/book/search-{}-{}.html'
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["aiqiyi_wenxue_search_offset"]["start"]
        _end = NOVEL_CONF["aiqiyi_wenxue_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["aiqiyi_wenxue_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["aiqiyi_wenxue_search_offset"]["start_page"] == 1:
    ...
    def get_qin_quan_id_int(self, qin_quan_url_str):
        try:
            return qin_quan_url_str.split('/')[-1].replace('detail-', '').replace('.html', '')
        except:
            return None
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//ul[@class="stacksList"]/li')  # TwoBox02_01
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('.//em[@class="writerName"]/text()'))  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('.//a[@class="js_highlight"]//text()'))  # 侵权标题
                qin_quan_url_str = "http:" + "".join(n_l.xpath('.//a[@class="js_highlight"]/@href'))  # 侵权链接
                ...
                novel_dict['qin_quan_platform_str'] = '爱奇艺文学'  # 侵权平台
