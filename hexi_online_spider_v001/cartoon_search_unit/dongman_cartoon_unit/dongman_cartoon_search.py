...
class DongManCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            # 'Referer': 'https://www.dongmanmanhua.cn/search?keyword=%E7%88%B1',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "https://www.dongmanmanhua.cn/search?keyword={}&page={}"
        self.qin_quan_info_url_pre = "https://www.dongmanmanhua.cn"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["dongman_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["dongman_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="card_lst"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./a/@href'))  # 侵权链接

                qin_quan_author_str = ''.join(q_l.xpath('./a/div[@class="info"]/p[@class="author"]/text()'))  # 侵权作者
                qin_quan_title_str = ''.join(q_l.xpath('./a/div[@class="info"]/p[@class="subj"]/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '咚漫'  # 侵权平台
                ...
                if qin_quan_dict["title_similar_number_float"] >= CARTOON_CONF["qin_quan_similar"] and qin_quan_dict[
                    ...
                    qin_quan_dict["qin_quan_flag_int"] = 1
                qin_quan_dict['all_recommend_str'] = ''.join(q_l.xpath('./a/div[@class="info"]/p[@class="grade_area"]/em[@class="grade_num"]/text()'))  # 总推荐数 str
                ...
                qin_quan_dict['book_lable_str'] = ''.join(q_l.xpath('./a/span[contains(@class,"genre")]/text()'))  # 书籍标签 （用｜分割的字符串 ''科幻｜现实｜励志''）
...
search_normals = DongManCartoon().search_qin_quan
