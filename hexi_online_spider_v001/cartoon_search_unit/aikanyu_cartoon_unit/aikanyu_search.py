...
class AiKanYu:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.aikanyu.com/vod-search-wd-{}-p-{}"
        self.qin_quan_search_url_pre = "http://www.aikanyu.com/"
        # self.data = {
        #     'key':'{}',  #.encode("gbk")
        #
        # }
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        ...
        # print(a)
        # exit(0)
        _page = kwargs.get("page_num") + CARTOON_CONF["aikanyu_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["aikanyu_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="thumbnail-group clearfix"]/li/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//ul[@class='detail-actor clearfix']/li/p/a")[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权作者

                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="detail-header clearfix"]/h2/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '爱看鱼'  # 侵权平台
...
search_normals = AiKanYu().search_qin_quan
