...
class piManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://www.manhuatao.com/search/?keywords={}&page={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "" #详情页链接不需要拼接

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["pimanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["pimanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('//*[@id="w0"]/ul/li/p[1]/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('/html/body/div[3]/div[1]/div[1]/div[2]')[0]
                qin_quan_author_str1 = qin_quan_info_temp.xpath('./ul[1]/li[1]/text()')[0]# 侵权作者
                qin_quan_author_str = qin_quan_author_str1.replace("作者：", "")
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '漫画淘'  # 侵权平台
...
search_normals = piManHuaCartoon().search_qin_quan
