...
class FeiLuNovel:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            'Origin': 'http://www.zhulang.com',
            'Content-Type': 'application/x-www-form-urlencoded',
            # 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.102 Safari/537.36',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
            'Referer': 'http://www.zhulang.com/search/index.html',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            # 'Cookie': 'PHPSESSID=1fbbcbdef1fdbe58c29b204f98a61ea0; Hm_lvt_c4d45d45e1a473a3e43c8699766169ee=1600680604,1600680724,1600936170,1600997142; Hm_lpvt_c4d45d45e1a473a3e43c8699766169ee=1600997381'
        }
    ...
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'http://www.zhulang.com/search/index.html'
        search_result = []
        if not search_key:
            return search_result
        # data = 'k={}'.format(self.get_unicode_key(search_key))
        data = {
            'k': search_key
        }
        # print(data)
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//div[@class="sch-res-list"]/ul/li') # TwoBox02_01
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('./dl/dd/h3/a/text()')[1])  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('./dl/dd/h3/a/text()')[0])  # 侵权标题
                qin_quan_url_str = "".join(n_l.xpath('./dl/dd/h3/a/@href')[0]).rstrip('/')  # 侵权链接
                qin_quan_id_int = self.get_qin_quan_id_int(qin_quan_url_str)  # 样本ID 数值类型
                qin_quan_mid_str = str(qin_quan_id_int)  # 侵权ID 字符串形式
                book_type_str = "".join(n_l.xpath('./dl/dd/p[@class="book-info"]/text()')).split('|')[0].replace(' ', '')
                ...
                novel_dict['qin_quan_platform_str'] = '逐浪小说'  # 侵权平台
