...
class MoTieNovel():
    def __init__(self, use_proxy=True):
        ...
        self.search_base_url = "https://app2.motie.com/search?pageNo={page}&pageSize=10&objectType=2&word={key_words}"  # 页码page从0开始

        self.novel_url_pre = "https://www.motie.com/book/"
    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["mo_tie_search_offset"]["start"]
        _end = NOVEL_CONF["mo_tie_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["mo_tie_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["mo_tie_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_dict.get('data', []).get('bookList',[]) if response_dict.get('data', []) else []
            for n_l in novel_list_data:
                ...
                novel_dict['qin_quan_platform_str'] = '磨铁中文网'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = MoTieNovel(use_proxy=True).search_novel
