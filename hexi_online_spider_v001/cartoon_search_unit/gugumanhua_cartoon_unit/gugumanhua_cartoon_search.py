...
class guguCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.gugumh.com/e/search/result/index.php?page={}&searchid={}"
        # print(self.search_base_url)
        self.qin_quan_search_url_pre = "http://www.gugumh.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["gugugmanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["gugugmanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//*[@id="dmList"]/ul/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./p/a/@href'))# 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//*[@id="intro_l"]')[0]
                # print(qin_quan_info_temp)
                qin_quan_author_str1 = ''.join(qin_quan_info_temp.xpath('./div[2]/p[2]/text()')) # 侵权作者
                qin_quan_author_str = qin_quan_author_str1.replace('\n', '')
                # print(qin_quan_author_str,111)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('//*[@id="intro_l"]/div[1]/h1/text()'))# 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '古古漫画'  # 侵权平台
                ...
                # qin_quan_dict['book_status_str'] = ''.join(qin_quan_info_temp.xpath('.//a[@href=/list/lianzai/]')[0].xpath('./text()'))  # 书籍状态 （连载，完结，暂无）
                # qin_quan_dict['book_property_str'] = ''  # 书籍属性 （免费，会员，限免）
                # qin_quan_dict['author_type_str'] = ''  # 作者类型 （金牌，签约，独立 默认无）
                # qin_quan_dict['book_lable_str'] = '|'.join(''.join([i for i in q_l.xpath('.//a[@href=/list/shaonian/]/text()') if i]).replace('--', '').split(' '))  # 书籍标签 （用｜分割的字符串 ''科幻｜现实｜励志''）
                # qin_quan_dict['book_type_str'] = ''  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                # qin_quan_dict['book_update_time'] = ''.join(qin_quan_info_temp.xpath('.//span[@class=js]/text()')[4].xpath('./text()'))  # 书籍更新日期 年-月-日
                # qin_quan_dict['book_zong_zhang_jie_int'] = ''  # 书籍总的章节 完结的，未完结就填目前的总章节
                # qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = ''.join(qin_quan_info_temp.xpath('./div[2]/ul/li[3]/span[2]/a')[0].xpath('./text()')) # 最新章节名称
...
search_normals = guguCartoon().search_qin_quan
