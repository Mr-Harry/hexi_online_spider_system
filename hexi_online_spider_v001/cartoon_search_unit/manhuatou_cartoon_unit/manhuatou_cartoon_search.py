...
class ManHua36:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Origin': 'http://mhtou.com',
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
            'Referer': 'http://mhtou.com/search/result',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://mhtou.com/search/result"
        self.qin_quan_info_url_pre = "http://mhtou.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        data = {'keyword':search_key}
        return self.parse_search_qin_quan(unify_requests(method='POST', url=self.search_base_url, headers=self.headers, data=data, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="sa-comic_show_list"]/ul/li[@class="list_item"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data[:CARTOON_CONF["manhuatou_search_offset"]["max_size"]]:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./div[@class="pic"]/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                try:
                    qin_quan_author_str = ''.join(qin_quan_info_data.xpath('.//div[@class="g-3-l"]/div/div/p')[0].xpath('./a/text()'))  # 侵权作者
                except:
                    qin_quan_author_str = ''
                qin_quan_title_str = ''.join(q_l.xpath('./div[@class="pic"]/a/@title'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画透'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = ''.join(qin_quan_info_data.xpath('.//div[@class="g-3-l"]/div/div/p')[1].xpath('./text()'))  # 书籍状态 （连载，完结，暂无）
                # qin_quan_dict['book_property_str'] = ''  # 书籍属性 （免费，会员，限免）
                qin_quan_dict['author_type_str'] = ''.join(qin_quan_info_data.xpath('.//div[@class="g-3-l"]/div/div/p')[2].xpath('./a/text()'))  # 作者类型 （金牌，签约，独立 默认无）
                qin_quan_dict['book_lable_str'] = ''.join(qin_quan_info_data.xpath('.//div[@class="g-3-l"]/div/div/p')[3].xpath('./a/text()'))  # 书籍标签 （用｜分割的字符串 ''科幻｜现实｜励志''）
                ...
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./div[@class="pic"]/a/img/@src'))  # 书籍封面 URL
...
search_normals = ManHua36().search_qin_quan
