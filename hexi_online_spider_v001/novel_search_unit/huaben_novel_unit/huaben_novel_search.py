...
class TaDuNovel:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            'Cookie': 'JSESSIONID=A2CB279B081D0C641A22511917F25B3B'}


    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'http://so.ihuaben.com/search?keyword={}&page={}'
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["huaben_search_offset"]["start"]
        _end = NOVEL_CONF["huaben_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["huaben_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["huaben_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//div[@class="container"]/div[@class="searchresult"]')
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('./div/p/span/a/text()'))  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('./div/h2/a//text()'))  # 侵权标题
                qin_quan_url_str = self.qin_quan_url_str_add("".join(n_l.xpath('./div/h2/a/@href'))) # 侵权链接
                qin_quan_id_int = self.get_qin_quan_id_int(qin_quan_url_str)  # 样本ID 数值类型
                qin_quan_mid_str = str(qin_quan_id_int)  # 侵权ID 字符串形式
                book_type_str = "".join(n_l.xpath('.//a[@class="text-warning"]/text()'))
                ...
                novel_dict['qin_quan_platform_str'] = '话本小说'  # 侵权平台
...
if __name__ == "__main__":
    ...
    result = search_novels('我喜欢你', **yangben_dict)
