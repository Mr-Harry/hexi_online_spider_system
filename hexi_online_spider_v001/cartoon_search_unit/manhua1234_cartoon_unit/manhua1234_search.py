...
class ManHUa1234Cartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.mh1234.com/search/?keywords={}&page={}"
        self.qin_quan_search_url_pre = "https://www.mh1234.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") - CARTOON_CONF["manhua1234_search_offset"]["start_page"] + 1 if kwargs.get("page_num") else CARTOON_CONF["manhua1234_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//li[@class="item-lg"]/p[@class="fl cover"]/a')
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = ''.join(q_l.xpath('./@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('.//div[@class="info"]/p/text()')[0])        # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="title"]/h1/text()')).replace(' ', '')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画1234'  # 侵权平台
...
search_normals = ManHUa1234Cartoon().search_qin_quan
