...
class GuFengCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.gufengmh8.com/search/?keywords={}"
        self.qin_quan_search_url_pre = "" #a连接里的详情页不用拼接

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["gufengmanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["gufengmanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@id="contList"]/li')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./a/@href'))# 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('/html/body/div[4]/div[1]/div/div[3]/div[1]/div')[0]
                # print(qin_quan_info_temp)
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./div[2]/ul/li[2]/span[2]/a/text()')) # 侵权作者
                # print(qin_quan_author_str,111)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[2]/div[1]/h1/span/text()'))# 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '古风漫画'  # 侵权平台
...
search_normals = GuFengCartoon().search_qin_quan
