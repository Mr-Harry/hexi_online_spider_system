...
class QiSanSanManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://m.733.so/statics/search.aspx?key={}"
        self.qin_quan_search_url_pre = "https://m.733.so"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["qisansan_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["erciyuan_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="col_3_1"]/li/a[@class="ImgA autoHeight"]')
            for q_l in qin_quaq_list_data:
                ...
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="Introduct_Sub autoHeight"]/h1/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '733漫画'  # 侵权平台
...
search_normals = QiSanSanManHua().search_qin_quan
