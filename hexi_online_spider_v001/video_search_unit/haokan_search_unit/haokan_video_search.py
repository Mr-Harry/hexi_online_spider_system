...
class HaoKanVideo:
    ...
    def __init__(self, use_proxy=True):
        self.search_video_url = "https://haokan.baidu.com/videoui/page/search?pn={1}&rn=10&_format=json&tab=video&query={0}"
        self.headers = {
            ...
            # 'Cookie': 'BAIDUID=86BF4725C1255426485B0FC1C121091B:FG=1; PC_TAB_LOG=search_result_page'
        }
    ...
    def search_songs(self, song_name = '', **kwargs) -> list:
        ...
        # 搜索页码search_page
        _start = config["haokan_video_search_offset"]["start_page"]
    ...
    def parse_search_video_song(self, response) -> list:
        ...
        try:
            ...
            result_list = []
            try:
                video_song_list_data = tree.get('data', {}).get('response', {}).get('list', [])
            except:
                video_song_list_data =[]
            for v_s in video_song_list_data:
                video_dict = dict()
                video_dict["video2_title"] = v_s.get("title", '')
...
search_songs = HaoKanVideo(use_proxy=True).search_songs
if __name__ == '__main__':
    ...
    print(HaoKanVideo().search_songs('班淑传奇', **kwags))
