...
class HeiYanNovel:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            'Referer': 'https://www.heiyan.com/',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            #'Cookie': '__secdyid=708ffe0d39f51d83ebed46c5fd126b922a0313dc9761ceb0021600771072; uc=wKgDPl9p1ABeJDOG3EkjAg=='
            }
    ...
    def search_novel(self, search_key: str, **kwargs):
        ...
        _start = NOVEL_CONF["heiyan_search_offset"]["start"]
        _end = NOVEL_CONF["heiyan_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["heiyan_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["heiyan_search_offset"]["start_page"] == 1:
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            if not response_dict.get('success', {}):
                return result_list
            novel_list_data = response_dict.get('data', {}).get('content') if response_dict.get('data', {}) else []
            for n_l in novel_list_data:
                qin_quan_author_str = n_l.get('authorname')  # 侵权作者
                qin_quan_title_str = n_l.get('name')  # 侵权标题
                qin_quan_url_str = self.novel_url_pre + str(n_l.get('id', ''))  # 侵权链接
                qin_quan_id_int = n_l.get('id')  # 样本ID 数值类型
                ...
                novel_dict['qin_quan_platform_str'] = '黑岩小说网'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = HeiYanNovel(use_proxy=True).search_novel
if __name__ == "__main__":
    ...
    result = search_novels('我的主讲', **yangben_dict)
