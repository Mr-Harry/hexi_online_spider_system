...
class ManHua36:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            # 'referer': 'https://www.36mh.net/search/?keywords=%E7%88%B1',
            'accept-language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "https://www.36mh.net/search/?keywords={}&page={}"
        # self.qin_quan_info_url_pre = "http://www.yn887.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhua36_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhua36_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="book-list clearfix"]/li[@class="item-lg"]')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                try:
                    qin_quan_author_str = ''.join(qin_quan_info_data.xpath('.//ul[@class="detail-list cf"]/li')[1].xpath('./span')[1].xpath('./a/text()'))  # 侵权作者
                except:
                    qin_quan_author_str = ''
                qin_quan_title_str = ''.join(q_l.xpath('./p[@class="ell"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '36漫画网'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = ''.join(qin_quan_info_data.xpath('.//ul[@class="detail-list cf"]/li[@class="status"]')[0].xpath('./span/a/text()')[0])  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_type_str'] = ''.join(qin_quan_info_data.xpath('.//ul[@class="detail-list cf"]/li')[1].xpath('./span/a/text()')[0])  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                ...
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('./a/span[@class="tt"]/text()'))  # 书籍简介 text
...
if __name__ == "__main__":
    ...
    # result = search_normals('爱丽丝的愿望', **yangben_dict) # 单一的剧情，xpath ok
    result = search_normals('派遣战斗员', **yangben_dict) # 剧情里 包含｜会定位出错 （已修改完成）
