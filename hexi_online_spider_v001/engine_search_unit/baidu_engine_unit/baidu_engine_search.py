...
from engine_search_unit.baidu_engine_unit._spider import BaseSpider
from engine_search_unit.baidu_engine_unit.parser import Parser
...
class BaiduSpider(BaseSpider):
    def __init__(self, use_proxy=True) -> None:
        ...
        self.headers = {
            ...
            'Cookie': 'BIDUPSID=0CF1222F861B9ADF1C174910B9E481FD; PSTM=1600678643; BAIDUID=CFA92C8BB73D02E6B5CA0CEA1C9C88A9:FG=1; BD_UPN=123353; BD_HOME=1; delPer=0; BD_CK_SAM=1; PSINO=2; BDORZ=B490B5EBF6F3CD402E515D22BCDA1598; COOKIE_SESSION=5_0_7_7_6_40_1_0_4_5_1_30_93859_0_0_0_1601562716_0_1601874177%7C9%2388026_3_1601223985%7C2; H_PS_PSSID=32813_32617_1433_32791_7545_31254_32795_32705_32230_7517_7605_32117_26350; H_PS_645EC=74f36VaX3zZvIH13LVCWiNn8oe7piPIePavJeU2qVV4phsSkQc3tfADsU0Q; BDSVRTM=14'
        }
        self.parser = Parser()

    def search_web_lpy(self, search_key: str, **kwargs) -> list:
        search_result = []
        if not search_key:
            return search_result
        _start = ENGINE_CONF["baidu_engine_offset"]["start_page"]
        if kwargs.get("page_num"):
            page = int(kwargs.get("page_num")) + ENGINE_CONF["baidu_engine_offset"]["start_page"] - 1
        else:
            page = _start
        web_result_temp = self.search_web(query=search_key, pn=page)
        web_result = web_result_temp.get('results', [])
        for i in web_result:
            if i.get('type') != 'related' and i.get('type') != 'total' and i.get('type') != 'baike':
                dict_unit = dict()
                dict_unit["qinquan_title"] = i.get('title', '')
                dict_unit["qinquan_text"] = i.get('des', '')
                dict_unit["qinquan_URL"] = i.get('url', '')
                # dict_unit["qinquan_url_hash"] = xx
                dict_unit["qinquan_platform"] = "百度搜索"
                if (not ('baidu.com/link' in dict_unit["qinquan_URL"])) and (dict_unit["qinquan_URL"]):
                    search_result.append(dict_unit)
        # return search_result
        return qinquan_list_url_clear(search_result)
    ...
    def search_pic(self, query: str, pn: int = 1) -> dict:
        ...
        try:
            url = 'http://image.baidu.com/search/flip?tn=baiduimage&word=%s&pn=%d' % (
                quote(query), (pn - 1) * 20)
            source = unify_requests(url=url, headers=self.headers)
            content = source.text
    ...
    def search_news(self, query: str, pn: int = 1) -> dict:
        ...
        for res in data:
            ...
            # 作者
            author = res.find('div', class_='c-span-last').find('div', class_='news-source').find('span',
                                                                                                  class_='c-gap-right').text
            # 发布日期
            date = res.find('div', class_='c-span-last').find('div', class_='news-source').find('span',
    ...
    def search_baike(self, query: str) -> dict:
        ...
        """
        # 获取源码
        source = unify_requests('https://baike.baidu.com/search?word=%s' % quote(query), headers=self.headers)
        code = self._minify(source.text)
        # 创建BeautifulSoup对象
...
search_engines = BaiduSpider(use_proxy=True).search_web_lpy

if __name__ == '__main__':
    import pprint

    result = search_engines(search_key='开局“十四五”牢记总书记对宣传思想战线的谆谆嘱托')
