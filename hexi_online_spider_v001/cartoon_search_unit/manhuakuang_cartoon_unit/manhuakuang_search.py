...
class ManHuaKuang:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.mhk8.com/search/{}/{}.html"
        self.qin_quan_search_url_pre = "https://www.mhk8.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhuakuang_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhuakuang_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="box04 l"]/ul/li/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('//div[@class="info l"]/ul/li/p/text()')[0])   # 侵权作者
                # if not qin_quan_author_str:
                #     continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="info l"]/ul/li/h2/text()')).replace(' ','').replace('\n','').replace('\r','')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画狂'  # 侵权平台
...
search_normals = ManHuaKuang().search_qin_quan
