...
class JinRICartoon:
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["jinri_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["jinri_search_offset"]["start_page"]
        return self.parse_search_qin_quan(unify_requests(url=self.search_base_url.format(int(_page)*20,quote(search_key+"+漫画") ), headers=self.headers, proxies=self.proxy), **kwargs)
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        ...
        for q_l in response.json()["data"]:
            if "host" in q_l and "cell_type" in q_l and q_l["cell_type"]==67:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = q_l.get("url","")  # 侵权链接
                qin_quan_author_str =  q_l.get("display", {}).get("data_ext",{}).get("author","") # 侵权作者
                # print(qin_quan_author_str)
                # if not qin_quan_author_str:
                #     continue
                qin_quan_title_str = q_l.get("title","")  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '头条站外搜索'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = q_l.get("display", {}).get("data_ext",{}).get("status","")  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_cover_image_str'] = q_l.get("display", {}).get("info",{}).get("images",[]) # 书籍封面 URL
                # print(qin_quan_dict['book_cover_image_str'])
                qin_quan_dict['book_cover_image_str'] = qin_quan_dict['book_cover_image_str'][0] if len(qin_quan_dict['book_cover_image_str'])>0 else "" # 书籍封面 URL
                # print("xyy",qin_quan_dict['book_cover_image_str'])
                ...
                # qin_quan_dict[''] = ''  #
                if clear_text(kwargs.get('yang_ben_title_str', '')) in clear_text(qin_quan_title_str):
                    result_list.append(qin_quan_dict)
        return result_list


# 统一的调用 search_qin_quans
search_normals = JinRICartoon().search_qin_quan
if __name__ == "__main__":
    yangben_dict = {
        'id': '10000',
        # 'yang_ben_author_str': '伊藤伸平神乐坂淳',
        'yang_ben_title_str': '我老婆是鬼王',
        'yang_ben_url_str': 'https://t.shuqi.com/cover/6695029',
        'page_num': 1,

    }
    result = search_normals('我老婆是鬼王', **yangben_dict)
