...
class JinJiangNovel:
    def __init__(self, use_proxy=True):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.jjwxc.net/onebook.php?novelid=47943',
            'Accept-Language': 'zh-CN,zh;q=0.9'
            }
        # self.api_novel_search_key = ''
        # self.search_base_url = "https://search.heiyan.com/web/search?queryString={}&highlight=false&page={}"
        # self.novel_url_pre = "http://www.ruochu.com/book/"

    def get_gbk_key(self, key):
        return str(key.encode('gbk')).lstrip("b'").rstrip("'").upper().replace("\X", "%")
    # 搜索小说

    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'http://www.jjwxc.net/search.php?kw={}&t=1&p={}'
        search_result = []
        if not search_key:
            return search_result
        search_key = self.get_gbk_key(search_key)
        _start = NOVEL_CONF["jinjiang_search_offset"]["start"]
        _end = NOVEL_CONF["jinjiang_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["jinjiang_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["jinjiang_search_offset"]["start_page"] == 1:
    ...
    def get_qin_quan_id_int(self, qin_quan_url_str):
        try:
            parms_list = [qin_quan_url_str.split('?')[-1].split('&')]
        except:
            return None
        for i in parms_list:
            if str(i).startswith('novelid='):
                try:
                    return int(str(i).replace('novelid=', '').replace(' ', ''))
                except:
                    return None
        else:
            return None
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//div[@id="search_result"]/div[not(@class)]')
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('./div[@class="info"]/a/span/text()')).replace(' ', '').replace('\r', '').replace('\n', '')  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('./h3[@class="title"]/a//text()'))  # 侵权标题
                qin_quan_url_str = "".join(n_l.xpath('./h3[@class="title"]/a/@href'))  # 侵权链接
                qin_quan_id_int = self.get_qin_quan_id_int(qin_quan_url_str)  # 样本ID 数值类型
                qin_quan_mid_str = None  # 侵权ID 字符串形式
                ...
                novel_dict['qin_quan_platform_str'] = '晋江小说'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = JinJiangNovel(use_proxy=True).search_novel
