...
class tukuManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "http://www.tuku.cc/search/index/nickname/{}/p/{}.html/"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "http://www.tuku.cc"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["tukumanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["tukumanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('/html/body/div[9]/div[2]/div/div[2]/div')
            # print(qin_quaq_list_data)
            # print(len(qin_quaq_list_data))
            # 循环列表
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./div/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                # qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="ib info"]/p/span/a/text()')
                if qin_quan_info_data.xpath('//div[@class="ib info"]/p/span/a'):
                    qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="ib info"]')[0]
                    # print(qin_quan_info_temp)
                    # exit(0)
                    qin_quan_author_str = qin_quan_info_temp.xpath('./p/span/a/text()')[0]  # 侵权作者
                else:
                    qin_quan_author_str = ''
                ...
                # if not qin_quan_author_str:
                    # continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./p[1]/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '图库漫画'  # 侵权平台
...
search_normals = tukuManHuaCartoon().search_qin_quan
