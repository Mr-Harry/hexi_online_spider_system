...
class haoManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://www.hao123.com/manhua/search?keyword={}&pn={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "https://www.hao123.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["haomanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["haomanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('//*[@id="bd"]/div[1]/div[1]/ul/li/div[2]/div[1]/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//*[@id="bd"]/div[1]/div[1]/div[2]/div[1]/div')[0]
                qin_quan_author_str = qin_quan_info_temp.xpath('./ul/li[1]/span[2]/text()')[0]# 侵权作者
                # qin_quan_author_str = ""
                # print(qin_quan_author_str,'作者')
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./div[1]/text()'))  # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '好123漫画'  # 侵权平台
...
search_normals = haoManHuaCartoon().search_qin_quan
