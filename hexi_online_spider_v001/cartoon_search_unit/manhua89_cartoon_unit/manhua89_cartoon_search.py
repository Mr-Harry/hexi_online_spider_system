...
class ManHua89Cartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.2020mh.com/search/q_%E7%88%B1',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://mip.89mh.com/statics/searchmip.aspx?key={}&page={}"
        self.qin_quan_info_url_pre = "http://mip.89mh.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhua89_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhua57_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class=" col_3_1 clearfix"]/li[@class="list-comic"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./a[@class="txtA"]/@href'))  # 侵权链接
                # qin_quan_author_str = ''.join(q_l.xpath('./div[@class="book-detail"]/dl/dd[@class="tags"]')[1].xpath('./span/a/text()'))  # 侵权作者
                qin_quan_dict = dict()
                try:
                    # response_data = json.loads(response.text)
                    qin_quan_info_html = self.qin_quan_info(qin_quan_url_str)
                    qin_quan_info_html.encoding=qin_quan_info_html.apparent_encoding
                    qin_quan_info_html = qin_quan_info_html.text
                    # print(qin_quan_info_html)
                    # exit(0)
                    qin_quan_info_data = etree.HTML(qin_quan_info_html)
                    # print(etree.tostring(qin_quan_info_data))
                    # print(response_dict)
                except:
                    continue
                # qin_quan_info_data = qin_quan_info_data.xpath('.//div[@class="product"]')[0]
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('//div[@class="pic"]/dl[@class="pic_zi fs15"]')[1].xpath('./dd[@class="left"]/text()'))  # 侵权作者
                qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = ''.join(qin_quan_info_data.xpath('//div[@class="pic"]/dl[@class="pic_zi fs15"]')[0].xpath('./dd[@class="left"]/text()'))  # 最新章节名称
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue

                # print(qin_quan_author_str)
                qin_quan_title_str = ''.join(q_l.xpath('./a[@class="txtA"]/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '89漫画网'  # 侵权平台
                ...
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./a[@class="ImgA"]/mip-img/@src'))  # 书籍封面 URL
...
search_normals = ManHua89Cartoon().search_qin_quan
