...
class LiuLiuManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.6mh7.com/search.php?keyword={}"
        self.qin_quan_search_url_pre = "http://www.6mh7.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") + CARTOON_CONF["wu22manhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["wu22manhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//div[@class='cy_xinxi']/span")[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()'))[3:] # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="cy_title"]/h1/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '66漫画'  # 侵权平台
...
search_normals = LiuLiuManHua().search_qin_quan
