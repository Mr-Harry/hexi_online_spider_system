...
class ManHuaTianTang:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.manhuatt.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_search_url_pre = "http://www.manhuatt.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhuatiantang_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhuatiantang_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="mh-search-list searchrest clearfix"]/li/div[@class="mh-worksbox"]/div[@class="mh-worksbord"]//div[@class="mh-nlook-w"]/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath('//p[@class="mh-pdt30 works-info-tc"]/span/em/a')[0]
                ...
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="mh-date-info-name"]/h4/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画天堂'  # 侵权平台
...
search_normals = ManHuaTianTang().search_qin_quan
