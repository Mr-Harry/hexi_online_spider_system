...
class shoujiManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "http://www.gmanhua.com/search?page={}&title={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "http://www.gmanhua.com"
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('//*[@id="search_nrl"]/div')
            # print(qin_quaq_list_data)
            # print(len(qin_quaq_list_data))
            # 循环列表
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = self.qin_quan_search_url_pre + ''.join(q_l.xpath('./div[1]/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                try:
                    qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="csly1z z"]/a')[2]
                    qin_quan_author_str = qin_quan_info_temp.xpath('./text()')[0]
                except Exception:
                    qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="csly1z z"]/a')[1]
                    qin_quan_author_str = qin_quan_info_temp.xpath('./text()')[0]
                if not qin_quan_author_str:
                    continue
                # qin_quan_title_str = ''.join(qin_quan_info_temp.xpath('./text()'))  # 侵权标题
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="csly1z z"]/h1/a/text()')) # 侵权标题
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '手机漫画'  # 侵权平台
...
search_normals = shoujiManHuaCartoon().search_qin_quan
