...
class BiKaBiKaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            # 'Referer': 'http://m.hanhande.net/search/?keywords=%E7%88%B1',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://m.hanhande.net/search/?keywords={}&page={}"
        # self.qin_quan_info_url_pre = "http://www.bikabika.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["hanhan_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["hanhan_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="UpdateList"]/div[@class="itemBox"]')
            # print(len(qin_quaq_list_data))
            for q_l in qin_quaq_list_data:
                # xpath_demo = ''.join(q_l.xpath(''))
                qin_quan_url_str = ''.join(q_l.xpath('./div[@class="itemTxt"]/a/@href'))  # 侵权链接
                qin_quan_author_str = ''.join(q_l.xpath('./div[@class="itemTxt"]/p[@class="txtItme"]')[0].xpath('./text()'))  # 侵权作者
                qin_quan_title_str = ''.join(q_l.xpath('./div[@class="itemTxt"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '憨憨漫画'  # 侵权平台
                ...
                qin_quan_dict['book_lable_str'] = ''.join(q_l.xpath('./div[@class="itemTxt"]/p[@class="txtItme"]')[1].xpath('./span[@class="pd"]/text()'))  # 书籍标签 （用｜分割的字符串 ''科幻｜现实｜励志''）
                # qin_quan_dict['book_type_str'] = ''  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                qin_quan_dict['book_update_time'] = ''.join(q_l.xpath('./div[@class="itemTxt"]/p[@class="txtItme"]')[2].xpath('./span[@class="date"]/text()'))  # 书籍更新日期 年-月-日
                qin_quan_dict['book_zong_zhang_jie_int'] = ''.join(q_l.xpath('./a[@class="coll"]/text()'))  # 书籍总的章节 完结的，未完结就填目前的总章节
                # qin_quan_dict['book_zui_xin_zhang_jie_name_str'] = ''  # 最新章节名称
                qin_quan_dict['book_introduce_text'] = ''  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./div[@class="itemImg"]/a/mip-img/@src'))  # 书籍封面 URL
...
search_normals = BiKaBiKaCartoon().search_qin_quan
