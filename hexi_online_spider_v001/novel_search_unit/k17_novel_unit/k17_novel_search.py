...
class K17Novel:
    ...
    def search_novel(self, search_key: str, **kwargs):
        ...
        _start = NOVEL_CONF["k17_search_offset"]["start"]
        _end = NOVEL_CONF["k17_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["k17_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["k17_search_offset"]["start_page"] == 1:
                _start = kwargs.get("page_num")
                _end = kwargs.get("page_num") + 1
        for page in range(_start, _end):
            search_url = f'https://search.17k.com/search.xhtml?c.q={search_key}&c.st=0&book_status=0&word_count=0&book_free=0&sort=0&fuzzySearchType=1&page={page}'
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        ...
        div_list = html_str.xpath('//*[@class="search-list"]/div[@class="textlist"]')[1:-1]
        for div in div_list:
            qin_quan_author_str = "".join(div.xpath('.//span[@class="ls"]/a/text()')).strip()
            qin_quan_url_str = "".join(div.xpath('./div[@class="textmiddle"]/dl/dt/a/@href'))
            if 'https' not in qin_quan_url_str:
                qin_quan_url_str = 'https:' + qin_quan_url_str
            else:
                qin_quan_url_str = qin_quan_url_str
            qin_quan_title_str = "".join(div.xpath('./div[@class="textmiddle"]/dl/dt/a/text()')).strip()
            qin_quan_id_int = int(qin_quan_url_str.split('/')[-1].split('.')[0].strip())
            qin_quan_mid_str = qin_quan_url_str.split('/')[-1].split('.')[0].strip()  # 侵权ID 字符串形式
            ...
            novel_dict['book_type_str'] = "".join(div.xpath('.//div[@class="textmiddle"]/dl/dd/ul/li')[0].xpath('./span/a/text()')[1]).strip()  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割  *****

            novel_dict['search_key_words_str'] = kwargs.get('search_key_words_str', '')  # 搜索关键词
            novel_dict['qin_quan_platform_str'] = '17k小说网'  # 侵权平台
...
search_novels = K17Novel(use_proxy=True).search_novel
