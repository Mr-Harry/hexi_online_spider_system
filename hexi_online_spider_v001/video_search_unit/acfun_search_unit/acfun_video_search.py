...
class AcfunVideo:
    ...
    def __init__(self, use_proxy=True):
        self.search_video_url = "https://www.acfun.cn/search?type=video&keyword={}&pCursor={}&sortType=1&quickViewId=video-list&ajaxpipe=1&channelId=0"
        self.headers = {
    ...
    def search_songs(self, song_name = '', **kwargs) -> list:
        ...
        # 搜索页码search_page
        _start = config["acfun_video_search_offset"]["start_page"]
    ...
    def parse_search_video_song(self, response) -> list:
        ...
        # print(response.url)
        try:
            tree = etree.HTML(json.loads(response.text.replace('/*<!-- fetch-stream -->*/','')).get('html', ''))
        except:
            return []
        else:
            result_list = []
            ...
            for v_s in video_song_list_data:
                video_dict = dict()
                video_dict["video2_title"] = "".join(v_s.xpath(".//div[@class='video__main']/div[@class='video__main__title ellipsis2']/a//text()"))
                if not self.check_video_title(video_dict.get("video2_title")):
                    continue
                video_dict["video2_url"] = "https://www.acfun.cn" + "".join(v_s.xpath(".//div[@class='video__main']/div[@class='video__main__title ellipsis2']/a/@href"))
                video_dict["video2_author"] = "".join(v_s.xpath(".//div[@class='video__main']/div[@class='video__main__info']//span[@class='user-name']/text()"))
                # video_dict["video_author_zone"] = "".join(v_s.xpath(".//a[@class='video-meta-user']/@href"))
                video_dict["video2_pubtime"] = "".join(v_s.xpath(".//span[@class='info__create-time']/text()"))
                video_dict["video2_url_hash"] = md5_use(video_dict.get("video2_url"))
                video_dict["video2_platform"] = "AcFun"
                duration_str_temp = "".join(v_s.xpath(".//span[@class='video__duration']/text()"))
...
search_songs = AcfunVideo(use_proxy=True).search_songs
if __name__ == '__main__':
    ...
    info = AcfunVideo().search_songs('班淑传奇', **kwags)
    print(info)
    print(len(info))