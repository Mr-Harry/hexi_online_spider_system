...
class ManHuaKu:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.mhku.net/search.asp?page={}&tag=0&key={}ysearchs"
        self.qin_quan_search_url_pre = "http://www.mhku.net"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhuaku_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhuaku_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="v_cont cf"]/dl[@class="v"]/dt[@class="v_pic"]/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath('//div[@class="eplisode_info cf"]/ul[@class="eplisode_meta"]/li/span[@class="cont"]')[1]
                ...
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//h1[@class="headerline_title"]/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画库'  # 侵权平台
...
search_normals = ManHuaKu().search_qin_quan
