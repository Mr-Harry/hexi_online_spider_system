...
class QinQinManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.acgcd.com/search/?keywords={}&page={}"
        self.qin_quan_search_url_pre = "https://www.77manhua.net"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["qinqinmanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["qinqinmanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="list_con_li update_con autoHeight"]/li/a[@class="image-link"]')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//ul[@class='comic_deCon_liT']/li/a")[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()')).replace(' ','').replace('\r','').replace('\n','')  # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="comic_deCon autoHeight"]/h1/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '亲亲漫画'  # 侵权平台
...
search_normals = QinQinManHua().search_qin_quan
