...
class FeiLuNovel:
    ...
    def get_unicode_key(self, key):
        return str(key.encode("unicode-escape")).replace("b'", '').replace('\\\\', '%')

    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_base_url = 'https://b.faloo.com/l/0/{1}.html?t=1&k={0}'
        search_result = []
        if not search_key:
            return search_result
        search_key = self.get_unicode_key(search_key)
        _start = NOVEL_CONF["feilu_search_offset"]["start"]
        _end = NOVEL_CONF["feilu_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["feilu_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["feilu_search_offset"]["start_page"] == 1:
    ...
    def get_qin_quan_id_int(self, qin_quan_url_str):
        try:
            return int(qin_quan_url_str.split('?')[-1].replace('.html'))
        except:
            return None
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            novel_list_data = response_data.xpath('//div[@class="TwoBox02_02"]') # TwoBox02_01
            # print(len(novel_list_data))
            for n_l in novel_list_data:
                qin_quan_author_str = "".join(n_l.xpath('.//div[@class="TwoBox02_09"]/span/a/text()'))  # 侵权作者
                qin_quan_title_str = "".join(n_l.xpath('.//div[@class="TwoBox02_08"]/h1/a/text()'))  # 侵权标题
                qin_quan_url_str = "https:" + "".join(n_l.xpath('.//div[@class="TwoBox02_08"]/h1/a/@href'))  # 侵权链接
                qin_quan_id_int = self.get_qin_quan_id_int(qin_quan_url_str)  # 样本ID 数值类型
                qin_quan_mid_str = str(qin_quan_id_int)  # 侵权ID 字符串形式
                book_type_str = "".join(n_l.xpath('.//div[@class="TwoBox02_05"]/span[@class="fontSize14andHui"]/a/text()'))
                ...
                novel_dict['qin_quan_platform_str'] = '飞卢小说'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = FeiLuNovel(use_proxy=True).search_novel
