...
class yaosanwujiuManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://m.1359mh.com/sort/?key={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "https://m.1359mh.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        # _page = kwargs.get("page_num") + CARTOON_CONF["yaosanwujiumanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["yaosanwujiumanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('/html/body/section/div[2]/div[2]/ul/li/div/h3/a')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('/html/body/section/div[2]/div[1]')[0]
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '1359漫画'  # 侵权平台
...
search_normals = yaosanwujiuManHuaCartoon().search_qin_quan
