...
class ChiYuManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.cymh8.com/search/soft.html?q={}&PageIndex={}"
        self.qin_quan_search_url_pre = "https://www.cymh8.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["chiyumanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["chiyumanhua_search_offset"]["start_page"]
        ...
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('//p[@class="info-text"]//text()')
                qin_quan_author_str = ''.join(qin_quan_info_temp[0] if qin_quan_info_temp else qin_quan_info_temp)  # 侵权作者
                if not qin_quan_author_str:
                    continue
                # qin_quan_title_str = ''.join(q_l.xpath('.//div[@class="p-title"]/text()')).replace(' ', '')  # 侵权标题
                qin_quan_title_str = ''.join(q_l.xpath('.//text()')).replace(' ', '')   .split() [0]
                ...
                qin_quan_dict['qin_quan_platform_str'] = '池鱼漫画'  # 侵权平台
...
search_normals = ChiYuManHuaCartoon().search_qin_quan
if __name__ == "__main__":
    ...
    result = search_normals('元', **yangben_dict)
