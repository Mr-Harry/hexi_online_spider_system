...
class BiQuGeManHua:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "http://www.qubiwo.com/so/?{}.html"
        self.qin_quan_search_url_pre = "http://www.qubiwo.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["biquge_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["erciyuan_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//ul[@class="flex"]/li/div[@class="img_span"]/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_info_temp = qin_quan_info_data.xpath("//div[@class='novel_info_title']/i/a")[0]
                qin_quan_author_str = ''.join(qin_quan_info_temp.xpath('./text()')).replace(' ','').replace('\r','').replace('\n','')  # 侵权作者
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="novel_info_title"]/h1/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '笔趣阁漫画'  # 侵权平台
...
search_normals = BiQuGeManHua().search_qin_quan
