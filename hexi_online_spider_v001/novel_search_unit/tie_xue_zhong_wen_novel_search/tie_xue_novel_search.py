...
class TieXueNovel():
    def __init__(self, use_proxy=True):
        ...
        self.search_base_url = "https://book.tiexue.net/SearchResults.aspx?pageindex={page}&keywords={key_words}&noveltype=0&PageSize=20"  # 页码page从0开始

        self.novel_url_pre = "https://book.tiexue.net"
    # 搜索小说
    def search_novel(self, search_key: str, **kwargs):
        search_result = []
        if not search_key:
            return search_result
        _start = NOVEL_CONF["tie_xue_search_offset"]["start"]
        _end = NOVEL_CONF["tie_xue_search_offset"]["end"]
        if kwargs.get("page_num"):
            if NOVEL_CONF["tie_xue_search_offset"]["start_page"] == 0:
                _start = kwargs.get("page_num") - 1
                _end = kwargs.get("page_num")
            elif NOVEL_CONF["tie_xue_search_offset"]["start_page"] == 1:
        ...
        for page in range(_start, _end):
            ...
            respose_text = unify_requests(url=self.search_base_url.format(key_words=search_key.encode('unicode_escape').decode().replace('\\u','%u'), page=page),headers={},proxies=self.proxy)
    ...
    def parse_search_novel(self, response, **kwargs) -> list:
        try:
            ...
            result_list = []
            book_list = select.xpath("//div[@class='tianZhi_list']//dl")
            for each in book_list:
                qin_quan_author_str = ''.join(each.xpath(".//dd//span[@class='zuoZhe1']/a//text()")).replace(" ","") # 侵权作者
                qin_quan_title_str =   ''.join(each.xpath(".//dd/h6/a//text()")).replace(" ","") # 侵权标题
                qin_quan_url_str = ''.join(each.xpath(".//dd/h6/a/@href")).replace(" ","")  # 侵权链接
                qin_quan_id_int = ""# 样本ID 数值类型

                qin_quan_mid_str = ''.join(re.findall(r'tiexue.net/(Book[0-9]*?)/',''.join(each.xpath(".//dd/h6/a//@href"))))   # 侵权ID 字符串形式
                ...
                novel_dict['qin_quan_platform_str'] = '铁血中文网'  # 侵权平台
...
# 统一的调用 search_novels
search_novels = TieXueNovel(use_proxy=True).search_novel
