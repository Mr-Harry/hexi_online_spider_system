...
class manmanManHuaCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        # url模板
        self.search_base_url = "https://m.manmantai.com/search/?keywords={}&page={}"


        # self.qin_quan_search_url_pre = "http://www.1manhua.net"
        # 详情页
        self.qin_quan_search_url_pre = "" #详情页不用拼接

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manmantaimanhua_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manmantaimanhua_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            # 所有书的列表
            qin_quaq_list_data = response_data.xpath('//*[@id="update_list"]/div/div/div[2]')
            ...
            for q_l in qin_quaq_list_data:
                ...
                try:
                    ...
                    continue
                qin_quan_info_temp = qin_quan_info_data.xpath('/html/body/div[2]/div[1]')[0]
                qin_quan_author_str = qin_quan_info_temp.xpath('./div/dl[2]/dd/text()')[0]# 侵权作者
                ...
                # 要改
                qin_quan_dict['qin_quan_platform_str'] = '慢慢睇漫画'  # 侵权平台
...
search_normals = manmanManHuaCartoon().search_qin_quan
