...
class YiNianCartoon:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.headers = {
            ...
            'Referer': 'http://www.yn887.com/statics/search.aspx?key=%E7%88%B1',
            'Accept-Language': 'zh-CN,zh;q=0.9',
            }
        self.api_qin_quan_search_key = ''
        self.search_base_url = "http://www.yn887.com/statics/search.aspx?key={}&page={}"
        self.qin_quan_info_url_pre = "http://www.yn887.com"

    # 搜索侵权
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["yinian_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["yinian_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_url_str = self.qin_quan_info_url_pre + ''.join(q_l.xpath('./dl/dt/a/@href'))  # 侵权链接
                ...
                try:
                    ...
                    continue
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('.//div[@class="w980 mt10 clearfix"]/div[@class="intro_l"]/div[@class="info"]/p')[1].xpath('./a/text()'))  # 侵权作者
                # print(qin_quan_author_str)
                if not qin_quan_author_str:
                    continue
                qin_quan_title_str = ''.join(q_l.xpath('./dl/dt/a/text()'))  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '一念漫画'  # 侵权平台
                ...
                qin_quan_dict['book_status_str'] = '|'.join(q_l.xpath('./dl/dd/p[@class="l2"]/span/text()'))  # 书籍状态 （连载，完结，暂无）
                ...
                qin_quan_dict['book_type_str'] = '|'.join(q_l.xpath('./dl/dd/p[@class="l2"]/a/text()'))  # 书籍分类 （玄幻 ,科幻，言情...）按搜索结果来多个按｜分割
                ...
                qin_quan_dict['book_introduce_text'] = ''.join(q_l.xpath('./dl/dd/p[@class="intro"]/text()'))  # 书籍简介 text
                qin_quan_dict['book_cover_image_str'] = ''.join(q_l.xpath('./p[@class="fl cover"]/a/img/@src'))  # 书籍封面 URL
...
search_normals = YiNianCartoon().search_qin_quan
