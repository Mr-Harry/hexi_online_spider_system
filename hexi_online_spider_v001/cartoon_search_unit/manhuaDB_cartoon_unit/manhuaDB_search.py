...
class ManHuaDB:
    def __init__(self, use_proxy=CARTOON_CONF.get('search_use_proxy')):
        ...
        self.search_base_url = "https://www.manhuadb.com/search?q={}&p={}"
        self.qin_quan_search_url_pre = "https://www.manhuadb.com"
    #转意函数
    ...
    def search_qin_quan(self, search_key: str, **kwargs):
        _page = kwargs.get("page_num") + CARTOON_CONF["manhuaDB_search_offset"]["start_page"] - 1 if kwargs.get("page_num") else CARTOON_CONF["manhuaDB_search_offset"]["start_page"]
    ...
    def parse_search_qin_quan(self, response, **kwargs) -> list:
        try:
            ...
            qin_quaq_list_data = response_data.xpath('//div[@class="row m-0"]/div[@class="col-4 col-sm-2 col-xl-1 px-1 px-md-1 px-lg-2 px-xl-1 pb-3"]/div/a')
            for q_l in qin_quaq_list_data:
                ...
                qin_quan_author_str = ''.join(qin_quan_info_data.xpath('//div[@class="comic-info p-3 py-sm-0 pr-sm-0 pl-sm-0"]/ul[@class="creators"]/li/a/text()'))   # 侵权作者
                # if not qin_quan_author_str:
                #     continue
                qin_quan_title_str = ''.join(qin_quan_info_data.xpath('//div[@class="comic-info p-3 py-sm-0 pr-sm-0 pl-sm-0"]/h1[@class="comic-title"]/text()')).replace(' ','').replace('\n','').replace('\r','')  # 侵权标题
                ...
                qin_quan_dict['qin_quan_platform_str'] = '漫画DB'  # 侵权平台
...
search_normals = ManHuaDB().search_qin_quan
